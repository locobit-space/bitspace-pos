...
const selectedCustomer = ref<typeof customersStore.customers.value[0] | null>(null);
...
const selectCustomer = (customer: typeof customersStore.customers.value[0]) => {
  selectedCustomer.value = customer;
  showCustomerModal.value = false;
  const toast = useToast();
  toast.add({ title: 'Customer Selected', description: customer.name || 'Customer', color: 'green' });
};
      ...
      <header
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-4">
            ...
            <!-- Currency Selector -->
            <USelect v-model="pos.selectedCurrency.value" :items="['LAK', 'THB', 'USD', 'SATS']" size="sm"
              class="w-24" />
        ...
        <div class="mt-3 flex gap-3">
          ...
          <div class="relative flex-1 max-w-md">
            <UInput v-model="productsStore.searchQuery.value" placeholder="Search products... (SKU or name)"
              icon="i-heroicons-magnifying-glass" size="sm" class="w-full" />
          </div>
    ...
    <div
        ...
        <div class="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
          <div v-if="selectedCustomer" class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              ...
              <div class="text-sm">
                <p class="font-medium text-gray-900 dark:text-white truncate max-w-[120px]">{{ selectedCustomer.name ||
                  'Customer' }}</p>
                <p class="text-xs text-gray-500">{{ selectedCustomer.phone || selectedCustomer.tier }}</p>
              </div>
            </div>
      ...
      <div
        ...
        <div class="space-y-2">
          ...
          <UButton block size="md" color="emerald" variant="soft" :disabled="pos.cartItems.value.length === 0"
            :loading="isProcessing" @click="sendToKitchen">
            <span class="flex items-center gap-2">
              <span class="text-lg">🔥</span>
              <span>{{ isEditingOrder ? "Update Order" : (t("pos.sendToKitchen") || "Send to Kitchen") }}</span>
              <span class="text-xs opacity-75">({{ isEditingOrder ? "Save Changes" : (t("pos.payLater") || "Pay Later")
              }})</span>
            </span>
          </UButton>
        </div>
            ...
            <div class="grid grid-cols-5 gap-2.5">
              ...
              <UButton color="emerald" variant="soft" size="lg" block
                ...
                <span class="flex items-center gap-2">
                  <span class="text-lg">🔥</span>
                  <span>{{ isEditingOrder ? "Update Order" : (t("pos.sendToKitchen") || "Send to Kitchen") }}</span>
                  <span class="text-xs opacity-75">({{ isEditingOrder ? "Save" : (t("pos.payLater") || "Pay Later")
                  }})</span>
                </span>
              </UButton>
            </div>
    ...
    <UModal v-model:open="showPaymentModal" :dismissable="!isProcessing">
      <template #content>
        <div class="p-6 bg-white dark:bg-gray-900 min-w-[400px] max-w-lg max-h-[85vh] overflow-y-auto">
          <PaymentSelector v-if="showPaymentModal"
        ...
        <div class="p-6 bg-white dark:bg-gray-900">
          ...
          <div class="space-y-4">
            <div>
              ...
              <div class="flex gap-2">
                <UButton :color="discountType === 'percentage' ? 'primary' : 'neutral'"
                  :variant="discountType === 'percentage' ? 'solid' : 'outline'" class="flex-1"
                  @click="discountType = 'percentage'">
                  % Percentage
                </UButton>
                <UButton :color="discountType === 'fixed' ? 'primary' : 'neutral'"
                  :variant="discountType === 'fixed' ? 'solid' : 'outline'" class="flex-1"
            ...
            <div>
              ...
              <UInput v-model.number="discountValue" type="number" :placeholder="discountType === 'percentage' ? 'e.g., 10' : 'e.g., 5000'
                " />
            </div>

            <div class="flex gap-2 pt-2">
              <UButton color="neutral" variant="outline" class="flex-1" @click="showDiscountModal = false">
                Cancel
              </UButton>
        ...
        <div class="p-6 bg-white dark:bg-gray-900">
          ...
          <div class="space-y-4">
            <div>
              ...
              <UInput v-model="customItem.name" placeholder="e.g., Special Order" />
            </div>
            ...
            <div>
              ...
              <UInput v-model.number="customItem.price" type="number" placeholder="e.g., 50000" />
            </div>

            <div class="flex gap-2 pt-2">
              <UButton color="neutral" variant="outline" class="flex-1" @click="showCustomItemModal = false">
                Cancel
              </UButton>
              <UButton color="primary" class="flex-1" :disabled="!customItem.name || customItem.price <= 0"
                @click="addCustomItem">
                Add to Cart
              </UButton>
            </div>
        ...
        <div class="p-6 bg-white dark:bg-gray-900">
          ...
          <div v-else class="space-y-3 max-h-96 overflow-auto">
            <div v-for="order in heldOrders" :key="order.id"
              ...
              <div class="flex gap-2">
                <UButton size="sm" color="primary" class="flex-1" @click="recallOrder(order.id)">
                  Recall
                </UButton>
                <UButton size="sm" color="red" variant="ghost" @click="deleteHeldOrder(order.id)">
        ...
        <div class="p-6 bg-white dark:bg-gray-900 min-w-[400px]">
          ...
          <div v-if="splitPaidCount > 0" class="mb-4">
            ...
            <div class="text-xs text-gray-500 mt-1 text-center">
              {{ currency.format(splitRemainingAmount, splitOrder?.currency || 'LAK') }} {{ t("pos.remaining") ||
                "remaining" }}
            </div>
          </div>
          ...
          <div class="flex gap-3">
            ...
            <UButton block color="primary" :loading="isProcessing"
        ...
        <div class="p-6 bg-white dark:bg-gray-900">
          ...
          <div class="grid grid-cols-3 gap-2">
            <button v-for="key in [
              ...
              '0',
              'DEL',
            ]" :key="key" class="h-14 rounded-xl font-bold text-lg transition-colors" :class="key === 'C'
          ...
          <UButton block size="lg" color="primary" class="mt-4" @click="handleNumpadInput('OK')">
            Confirm
          </UButton>
        </div>
      </template>
        ...
        <div class="p-6 bg-white dark:bg-gray-900 overflow-y-auto">
          ...
          <div class="space-y-6">
            ...
            <div>
              ...
              <USelect v-model="pos.selectedCurrency.value" :items="['LAK', 'THB', 'USD', 'SATS']" />
            </div>
        ...
        <div class="p-6 bg-white dark:bg-gray-900 max-h-[80vh] overflow-auto">
          <div v-if="selectedProduct" class="space-y-5">
            ...
            <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
              ...
              <div class="flex gap-2">
                <UButton color="neutral" variant="outline" class="flex-1" @click="showProductOptionsModal = false">
                  Cancel
                </UButton>
                <UButton color="primary" class="flex-1 bg-linear-to-r from-amber-500 to-orange-500"
      ...
      <template #content>
        <div class="p-6 bg-white dark:bg-gray-900">
          ...
          <div class="space-y-4">
            ...
            <UTextarea v-model="itemNotesValue" placeholder="Enter notes for this item..." :rows="3" autofocus />

            <!-- Quick Notes -->
            <div>
              <p class="text-xs text-gray-500 mb-2">Quick notes:</p>
              <div class="flex flex-wrap gap-2">
                <button v-for="quickNote in [
                  'No ice',
                  'Extra spicy',
                  'Less sugar',
                  'No onions',
            ...
            <div class="flex gap-2 pt-2">
              <UButton color="neutral" variant="outline" class="flex-1" @click="showItemNotesModal = false">
                Cancel
              </UButton>
      ...
      <template #content>
        <div class="p-6 max-h-[80vh] overflow-auto">
          ...
          <div v-if="tables.length > 0" class="grid grid-cols-3 sm:grid-cols-4 gap-3">
            <button v-for="table in tables" :key="table.id" :disabled="table.status === 'occupied' &&
              ...
              <!-- Table name -->
              <span class="font-semibold text-sm" :class="table.name === pos.tableNumber.value
                ? 'text-white'
              ...
              <!-- Seats -->
              <span class="text-xs" :class="table.name === pos.tableNumber.value
                ? 'text-white/75'
                : 'text-gray-500'
                ">
                {{ table.seats }} seats
              ...
              <span v-if="
                table.status !== 'available' &&
                table.name !== pos.tableNumber.value
              " class="absolute top-1 right-1 text-[10px] font-medium px-1.5 py-0.5 rounded-full" :class="table.status === 'reserved'
              ...
              <span v-if="table.status === 'occupied' && tablesStore.getTableOccupiedMinutes(table.id) > 0"
              ...
              <!-- Current indicator -->
              <span v-if="table.name === pos.tableNumber.value"
          ...
          <div v-else class="text-center py-8">
            ...
            <UButton color="primary" variant="soft" class="mt-4" @click="navigateTo('/pos/tables')">
              Set up Tables
            </UButton>
          </div>
          ...
          <div class="flex gap-2 mt-6 pt-4 border-t border-gray-200 dark:border-gray-700">
            <UButton color="neutral" variant="outline" class="flex-1" @click="showTableSwitcher = false">
              Cancel
            </UButton>
            <UButton color="primary" variant="soft" icon="i-heroicons-arrow-top-right-on-square"
        ...
        <div class="p-6">
          ...
          <div class="max-h-64 overflow-y-auto space-y-2">
            <div v-for="customer in filteredCustomers" :key="customer.id"
              ...
              <div class="flex-1 min-w-0">
                <p class="font-medium text-gray-900 dark:text-white truncate">{{ customer.name || 'Customer' }}</p>
          ...
          <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 flex gap-3">
            <UButton variant="outline" color="gray" class="flex-1" @click="showCustomerModal = false">Cancel</UButton>

...
const statusOptions = [
  { value: "pending", label: t("orders.status.pending") || "Pending", color: "amber" },
  { value: "processing", label: t("orders.status.processing") || "Processing", color: "blue" },
  { value: "completed", label: t("orders.status.completed") || "Completed", color: "green" },
  { value: "cancelled", label: t("orders.status.cancelled") || "Cancelled", color: "gray" },
  { value: "refunded", label: t("orders.status.refunded") || "Refunded", color: "red" },
];
...
const paymentStatus = computed(() => {
  ...
  if (totalPaid.value > 0 && totalPaid.value < order.value.total) return "partial";
  return "pending";
});
...
const markAsPaid = async () => {
  ...
  isSaving.value = true;
  try {
    await ordersStore.completeOrder(order.value.id, order.value.paymentMethod || "cash");
...
const cancelOrder = async () => {
  if (!order.value) return;
  if (!confirm(t("orders.cancelConfirmation") || "Are you sure you want to cancel this order?")) return;
...
<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-950">
    ...
    <!-- Order Not Found -->
    <div v-else-if="!order" class="flex flex-col items-center justify-center h-96">
          ...
          <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div class="flex items-center gap-4">
              ...
              <div>
                ...
                <div class="flex items-center gap-3 mt-1 text-sm text-gray-500 dark:text-gray-400 flex-wrap">
                  <span>{{ t("orders.ordered") || "Ordered" }}: {{ formatShortDate(order.date) }}</span>
                  <span>•</span>
                  <span>📦 {{ itemCount }} {{ t("pos.items") || "items" }}</span>
                  <span v-if="order.tableNumber">•</span>
                  <span v-if="order.tableNumber">🪑 {{ t("orders.table") || "Table" }} {{ order.tableNumber }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="flex items-center gap-2 flex-wrap">
              ...
              <UDropdown 
                ...
                :items="[
                  [
                    { label: 'Mark as Paid', icon: 'i-heroicons-check-circle', click: markAsPaid },
                    { label: 'Cancel Order', icon: 'i-heroicons-x-circle', click: cancelOrder }
                  ]
                ]"
              >
                <UButton color="primary" trailing-icon="i-heroicons-chevron-down">
          ...
          <div class="flex-1 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                  <!-- Customer Avatar -->
                  <div class="w-12 h-12 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center text-white font-bold text-lg shadow-lg">
                    {{ customerInitials }}
                  </div>
                  <div>
                    <div class="flex items-center gap-2">
                      ...
                      <UButton variant="link" size="xs" icon="i-heroicons-arrow-up-right" class="text-gray-400" />
                    </div>
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
              ...
              <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-800">
                <h3 class="font-semibold text-gray-900 dark:text-white">
                  {{ t("orders.items") || "Order Items" }} ({{ order.items?.length || 0 }})
                </h3>
              </div>
              ...
              <div class="divide-y divide-gray-100 dark:divide-gray-800">
                ...
                >
                  <!-- Product Image -->
                  <div class="w-14 h-14 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-800 flex-shrink-0 shadow-sm">
                    <img
                  ...
                  <div class="text-center px-4">
                    <span class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-gray-100 dark:bg-gray-800 text-sm font-medium">
                      {{ item.quantity }}x
                    </span>
                  </div>
              ...
              <div class="bg-gray-50 dark:bg-gray-800/50 px-5 py-4 space-y-2">
                <div class="flex justify-between text-sm">
                  <span class="text-gray-500">{{ t("pos.subtotal") || "Subtotal" }}</span>
                ...
                <div v-if="totalTax" class="flex justify-between text-sm">
                  ...
                  <span class="text-gray-900 dark:text-white">{{ formatCurrency(totalTax) }}</span>
                </div>
                <div v-if="totalDiscount" class="flex justify-between text-sm">
                  <span class="text-gray-500">{{ t("pos.discount") || "Discount" }}</span>
                ...
                <div v-if="totalTip" class="flex justify-between text-sm">
                  ...
                  <span class="text-gray-900 dark:text-white">{{ formatCurrency(totalTip) }}</span>
                </div>
                <div class="flex justify-between text-lg font-bold pt-2 border-t border-gray-200 dark:border-gray-700">
                  <span class="text-gray-900 dark:text-white">{{ t("pos.total") || "Total" }}</span>
                  <span class="text-primary-600 dark:text-primary-400">{{ formatCurrency(order.total) }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
              ...
              <UTextarea
                ...
                :rows="3"
                :placeholder="t('orders.notesPlaceholder') || 'Add notes about this order...'"
                class="w-full"
              />
          ...
          <div class="w-full lg:w-80 space-y-4">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
              ...
              <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
                ...
                >
                  ● {{ paymentStatus === 'paid' ? 'Paid' : paymentStatus === 'partial' ? 'Partially paid' : 'Pending' }}
                </span>
              </div>
              ...
              <div class="flex border-b border-gray-200 dark:border-gray-800">
                <button
                  ...
                  :class="activeSidebarTab === 'payments' 
              ...
              <div v-if="activeSidebarTab === 'payments'" class="p-5 space-y-4">
                ...
                <div class="flex items-center gap-3">
                  ...
                  <div class="flex-1">
                    <p class="text-sm font-medium text-gray-900 dark:text-white">Invoice</p>
                    <p class="text-xs text-gray-500">INV-{{ order.id.slice(-6).toUpperCase() }}</p>
                  </div>
                  <p class="font-semibold text-gray-900 dark:text-white">
                ...
                <div v-if="order.status === 'completed'" class="space-y-3">
                  <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 flex items-center gap-2">
                    ...
                    <span class="bg-gray-100 dark:bg-gray-800 text-xs px-1.5 py-0.5 rounded">1</span>
                  </h4>
                ...
                <div class="pt-4 border-t border-gray-200 dark:border-gray-700 space-y-2">
                  <div class="flex justify-between text-sm">
                    <span class="text-gray-500">{{ t("orders.paid") || "Paid" }}</span>
                    <span class="text-gray-900 dark:text-white">{{ formatCurrency(totalPaid) }}</span>
                  </div>
                  <div class="flex justify-between text-sm">
                    <span class="text-gray-500">{{ t("orders.outstanding") || "Outstanding" }}</span>
                    <span :class="outstandingAmount > 0 ? 'text-red-600 font-semibold' : 'text-gray-900 dark:text-white'">
              ...
              <div v-if="activeSidebarTab === 'notes'" class="p-5">
                <p v-if="order.notes" class="text-sm text-gray-600 dark:text-gray-400">
                  {{ order.notes }}
                </p>
              ...
              <div class="space-y-4">
                ...
                <div v-if="order.status === 'completed'" class="flex gap-3">
                  ...
                  <div class="flex-1">
                    ...
                    <p class="text-xs text-gray-500">{{ formatDate(order.updatedAt || order.date) }}</p>
                  </div>
                </div>

...
async function handleRenew(membership: Membership) {
  const plan = membershipsStore.plans.value.find((p) => p.id === membership.planId);
  const customer = customersStore.customers.value.find((c) => c.id === membership.customerId);
  ...
  // Store pending data and show payment modal
  pendingMembershipData.value = {
    customer: customer || { id: membership.customerId, name: membership.customerName },
    plan,
...
async function processPayment(method: 'cash' | 'lightning' | 'bank_transfer') {
  ...
  const { customer, plan, autoRenew, notes, isRenewal } = pendingMembershipData.value;
...
async function savePlan() {
  ...
  const benefitsLao = planForm.value.benefitsLao.split('\n').filter(b => b.trim());
    ...
    <UModal v-model:open="showPlanModal">
      <template #header>
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
          {{ selectedPlan ? (t('common.edit') || 'Edit') : (t('common.add') || 'Add') }} {{ t('memberships.plan') || 'Plan' }}
        </h3>
      </template>
      <template #body>
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <UFormField :label="(t('products.duration') || 'Duration') + ' (' + (t('common.days') || 'days') + ')'" required>
        ...
        <div v-if="pendingMembershipData" class="space-y-6">
          ...
          <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
            <div class="flex justify-between items-center mb-2">
              <span class="text-gray-600 dark:text-gray-400">{{ pendingMembershipData.isRenewal ? 'Renewal' : 'New Membership' }}</span>
              <UBadge color="primary">{{ pendingMembershipData.plan.name }}</UBadge>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-gray-600 dark:text-gray-400">{{ t('customers.name') || 'Customer' }}</span>
              <span class="font-medium text-gray-900 dark:text-white">{{ pendingMembershipData.customer.name }}</span>
            </div>
            <div class="flex justify-between items-center mt-2">
              <span class="text-gray-600 dark:text-gray-400">{{ t('products.duration') || 'Duration' }}</span>
              <span class="font-medium text-gray-900 dark:text-white">{{ pendingMembershipData.plan.duration }} {{ t('common.days') || 'days' }}</span>
            </div>
            <div class="border-t border-gray-200 dark:border-gray-700 mt-4 pt-4">
              <div class="flex justify-between items-center">
                <span class="text-lg font-semibold text-gray-900 dark:text-white">{{ t('pos.total') || 'Total' }}</span>
                <span class="text-2xl font-bold text-primary-600 dark:text-primary-400">{{ formatCurrency(pendingMembershipData.plan.price) }}</span>
              </div>
            </div>
          </div>
          ...
          <!-- Payment Methods -->
          <div class="space-y-3">
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('payment.selectMethod') || 'Choose payment method' }}:</p>
            ...
            >
              ...
              <div class="flex-1 text-left">
                <p class="font-semibold text-gray-900 dark:text-white">{{ t('payment.methods.cash') || 'Cash' }}</p>
                <p class="text-sm text-gray-500">{{ t('payment.methods.cashDesc') || 'Pay with cash' }}</p>
              </div>
            ...
            >
              ...
              <div class="flex-1 text-left">
                <p class="font-semibold text-gray-900 dark:text-white">{{ t('payment.methods.lightning') || 'Lightning' }}</p>
                <p class="text-sm text-gray-500">{{ t('payment.methods.lightningDesc') || 'Pay with Bitcoin Lightning' }}</p>
              </div>
              <UIcon name="i-heroicons-chevron-right" class="w-5 h-5 text-gray-400" />
            </button>
            ...
            >
              ...
              <div class="flex-1 text-left">
                <p class="font-semibold text-gray-900 dark:text-white">{{ t('payment.methods.bankTransfer') || 'Bank Transfer' }}</p>
                <p class="text-sm text-gray-500">{{ t('payment.methods.bankTransferDesc') || 'Pay via bank account' }}</p>
              </div>
              <UIcon name="i-heroicons-chevron-right" class="w-5 h-5 text-gray-400" />
            </button>
          </div>
          ...
          <div v-if="isProcessingPayment" class="flex items-center justify-center gap-3 py-4">
            ...
            <span class="text-gray-600 dark:text-gray-400">{{ t('pos.processing') || 'Processing...' }}</span>
          </div>
        </div>
      </template>
      <template #footer>

        ...
        <div class="flex flex-col md:flex-row gap-6">
          ...
          >
            ...
            <div class="text-center text-sm">
              ...
              <p class="font-mono">
                {{ customerRecord?.cardUid || "Not Assigned" }}
              </p>
            </div>
          </div>
          <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <p
                :class="remainingDays < 7 ? 'text-red-500' : 'text-green-500'"
                class="font-medium"
              >
                {{ remainingDays }} days
              </p>
        ...
        <UCard :ui="{ body: { padding: 'p-0' } }">
          ...
          <div class="hidden md:block overflow-x-auto">
            <table class="w-full text-sm text-left">
              <thead
                class="bg-gray-50 dark:bg-gray-800/50 text-gray-500 font-medium"
              >
                <tr>
          ...
          <div class="md:hidden divide-y divide-gray-100 dark:divide-gray-800">
            ...
            >
              <div class="flex justify-between items-start">
                <div>
                  <p class="text-xs text-gray-400 uppercase font-medium">
                    Check In
                  </p>
                  <p class="font-medium">
                    {{ formatDateTime(row.checkInTime) }}
              ...
              <div
                v-if="row.notes"
                class="text-sm text-gray-500 bg-gray-50 dark:bg-gray-800 p-2 rounded"
              >
                {{ row.notes }}
              </div>
            </div>
            <div
              v-if="memberCheckIns.length === 0"
        ...
        <div class="p-6">
          ...
          <div class="flex justify-end gap-2">
            <UButton
              color="neutral"
              variant="ghost"
              @click="showAssignCardModal = false"
              >Cancel</UButton
            >
...
const membership = computed(() =>
  memberships.value.find((m) => m.id === membershipId),
);
...
async function confirmAssignCard() {
  if (membership.value && cardUidInput.value) {
    const success = await assignCard(
      membership.value.customerId,
      cardUidInput.value,
    );
    if (success) {
      // Refresh customer record locally
      customerRecord.value = await db.customers.get(
        membership.value.customerId,
      );

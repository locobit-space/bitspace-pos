    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 px-4">
      <div class="flex items-center gap-4">
        ...
        <div v-if="product" class="flex items-center gap-4">
          <div class="w-16 h-16 bg-gray-100 dark:bg-gray-800 rounded-xl flex items-center justify-center overflow-hidden border-2 border-gray-200 dark:border-gray-700">
            <img
            ...
            >
            <span v-else-if="product.image" class="text-3xl">{{ product.image }}</span>
      ...
      <UTabs v-model="activeTab" :items="tabs">
        <template #content="{ item }">
          ...
          <div v-if="item.value === 'overview'" class="pt-6 space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
                ...
                <dl class="space-y-3">
                  ...
                  <div class="flex justify-between">
                    <dt class="text-gray-500">{{ $t('products.productType') }}</dt>
                  ...
                  <div v-if="product.manufacturer" class="flex justify-between">
                    <dt class="text-gray-500">{{ $t('products.manufacturer') }}</dt>
                    <dd class="font-medium text-gray-900 dark:text-white">
                      {{ product.manufacturer }}
                    </dd>
                  </div>
                  <div v-if="product.description" class="pt-2">
                    <dt class="text-gray-500 mb-1">{{ $t('products.description') }}</dt>
              ...
              <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
                ...
                <dl class="space-y-3">
                  <div class="flex justify-between">
                    <dt class="text-gray-500">{{ $t('products.trackStock') }}</dt>
                    <dd>
                      <UBadge
                        :color="product.trackStock !== false ? 'green' : 'gray'"
                        :label="product.trackStock !== false ? $t('common.yes') : $t('common.no')"
                      />
                    </dd>
                  </div>
                  <div class="flex justify-between">
                    <dt class="text-gray-500">{{ $t('products.trackLots') }}</dt>
                    <dd>
                      <UBadge
                        :color="product.trackLots ? 'green' : 'gray'"
                        :label="product.trackLots ? $t('common.yes') : $t('common.no')"
                      />
                    </dd>
                  </div>
                  <div class="flex justify-between">
                    <dt class="text-gray-500">{{ $t('products.hasExpiry') }}</dt>
                    <dd>
                      <UBadge
                        :color="product.hasExpiry ? 'yellow' : 'gray'"
                        :label="product.hasExpiry ? $t('common.yes') : $t('common.no')"
                      />
                    </dd>
                  </div>
                  <div v-if="product.hasExpiry && product.defaultShelfLifeDays" class="flex justify-between">
                    <dt class="text-gray-500">{{ $t('products.shelfLife') }}</dt>
                    <dd class="font-medium text-gray-900 dark:text-white">
                      {{ product.defaultShelfLifeDays }} {{ $t('common.days') }}
                    </dd>
                  </div>
                  <div v-if="product.storageType" class="flex justify-between">
                    <dt class="text-gray-500">{{ $t('products.storageType') }}</dt>
                    <dd class="font-medium text-gray-900 dark:text-white capitalize">
                      {{ product.storageType }}
                    </dd>
                  </div>
            ...
            <!-- Tags -->
            <div v-if="product.tags && product.tags.length > 0" class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6">
              <h3 class="font-semibold text-gray-900 dark:text-white mb-4">
                {{ $t('products.tags') }}
              </h3>
            ...
            <div class="bg-gray-50 dark:bg-gray-900/50 rounded-xl p-4 text-sm text-gray-500">
              <div class="flex flex-wrap gap-x-6 gap-y-2">
                <span>{{ $t('common.createdAt') }}: {{ formatDate(product.createdAt) }}</span>
                <span v-if="product.createdBy">{{ $t('common.createdBy') }}: {{ product.createdBy }}</span>
                <span>{{ $t('common.updatedAt') }}: {{ formatDate(product.updatedAt) }}</span>
          ...
          <div v-if="item.value === 'inventory'" class="pt-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <CommonStatCard
                ...
                :value="String(product.stock)"
                icon="i-heroicons-cube"
                :icon-color="product.stock <= product.minStock ? 'red' : 'green'"
              />
              <CommonStatCard
                :label="$t('products.minStock')"
                :value="String(product.minStock)"
              ...
              <CommonStatCard
                ...
                :value="formatCurrency(product.stock * (product.costPrice || product.price))"
            ...
            <div v-if="product.trackLots" class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
                ...
                <table class="w-full">
                  <thead>
                    <tr class="bg-gray-50 dark:bg-gray-800/50">
                      <th class="text-left py-3 px-4 text-sm font-medium text-gray-500">{{ $t('inventory.lotNumber') }}</th>
                      <th class="text-left py-3 px-4 text-sm font-medium text-gray-500">{{ $t('inventory.quantity') }}</th>
                      <th class="text-left py-3 px-4 text-sm font-medium text-gray-500">{{ $t('inventory.expiryDate') }}</th>
                      <th class="text-left py-3 px-4 text-sm font-medium text-gray-500">{{ $t('inventory.receivedDate') }}</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr
                    ...
                    >
                      ...
                      <td class="py-3 px-4 text-gray-500">{{ formatDate(lot.receivedDate) }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <div v-if="log.changes && log.changes.length > 0" class="mt-3 space-y-2">
                      ...
                      >
                        ...
                        <span class="text-red-500 line-through mx-2">{{ formatChangeValue(change.oldValue) }}</span>
                        <UIcon name="i-heroicons-arrow-right" class="w-3 h-3 inline text-gray-400" />
                        <span class="text-green-500 ml-2">{{ formatChangeValue(change.newValue) }}</span>
                      </div>
                    </div>

                    <!-- Stock change -->
                    <div v-if="log.action === 'stock_adjust' && log.stockBefore !== undefined" class="mt-2 text-sm">
                      <span class="text-gray-500">{{ $t('products.stock') }}:</span>
                      <span class="text-red-500 mx-2">{{ log.stockBefore }}</span>
                      <UIcon name="i-heroicons-arrow-right" class="w-3 h-3 inline text-gray-400" />
                      <span class="text-green-500 ml-2">{{ log.stockAfter }}</span>
                      <span v-if="log.stockReason" class="text-gray-500 ml-2">({{ log.stockReason }})</span>
                    </div>

                    <!-- Price change -->
                    <div v-if="log.action === 'price_change' && log.priceBefore !== undefined" class="mt-2 text-sm">
                      <span class="text-gray-500">{{ $t('products.price') }}:</span>
                      <span class="text-red-500 mx-2">{{ formatCurrency(log.priceBefore) }}</span>
                      <UIcon name="i-heroicons-arrow-right" class="w-3 h-3 inline text-gray-400" />
                      <span class="text-green-500 ml-2">{{ formatCurrency(log.priceAfter!) }}</span>
                    </div>
            ...
            <div v-if="linkedRecipes.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              >
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <h4 class="font-medium text-gray-900 dark:text-white">{{ recipe.name }}</h4>
      ...
      <template #content>
        <div class="p-6">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">
            {{ stockAdjustmentType === 'add' ? $t('inventory.addStock') : $t('inventory.removeStock') }}
          </h3>
          <div class="space-y-4">
            <UFormField :label="$t('inventory.quantity')">
              <UInput
                v-model.number="stockAdjustmentQty"
                type="number"
                min="0"
...
// Linked data
const stockLots = ref<{ id: string; lotNumber: string; quantity: number; expiryDate?: string; receivedDate: string }[]>([]);
const linkedRecipes = ref<{ id: string; name: string; icon?: string; category?: string }[]>([]);

// Permissions
const canEditProducts = computed(() => permissions.currentUser.value?.permissions?.canEditProducts ?? false);
const canDeleteProducts = computed(() => permissions.currentUser.value?.permissions?.canDeleteProducts ?? false);

// Tabs
const tabs = computed(() => [
  { label: t('products.overview'), value: 'overview', icon: 'i-heroicons-information-circle' },
  { label: t('products.inventory'), value: 'inventory', icon: 'i-heroicons-cube' },
  { label: t('products.history'), value: 'history', icon: 'i-heroicons-clock' },
  { label: t('products.recipes'), value: 'recipes', icon: 'i-heroicons-beaker' },
]);

const historyFilterOptions = [
  { label: t('common.all'), value: 'all' },
  { label: t('products.activityCreate'), value: 'create' },
  { label: t('products.activityUpdate'), value: 'update' },
  { label: t('products.activityPriceChange'), value: 'price_change' },
  { label: t('products.activityStockAdjust'), value: 'stock_adjust' },
  { label: t('products.activityDelete'), value: 'delete' },
];

const stockReasonOptions = [
  { label: t('inventory.reasonReceived'), value: 'received' },
  { label: t('inventory.reasonReturned'), value: 'returned' },
  { label: t('inventory.reasonDamaged'), value: 'damaged' },
  { label: t('inventory.reasonExpired'), value: 'expired' },
  { label: t('inventory.reasonLost'), value: 'lost' },
  { label: t('inventory.reasonAdjustment'), value: 'adjustment' },
  { label: t('inventory.reasonTransfer'), value: 'transfer' },
...
async function loadProduct() {
  loading.value = true;
  try {
    product.value = getProduct(productId.value) || null;
    if (product.value) {
      // Load related data
      await Promise.all([
        loadStockLots(),
...
async function loadActivityHistory() {
  ...
  try {
    const logs = await getProductActivityLogs(productId.value, historyLimit.value);
...
async function handleStockAdjustment() {
  ...
  try {
    const newStock = stockAdjustmentType.value === 'add'

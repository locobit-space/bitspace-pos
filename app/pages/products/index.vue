<template>
  <div class="container mx-auto p-6 space-y-6">
      ...
      <UCard>
        <template #header>
          <h3 class="text-lg font-medium">
            {{ selectedProduct?.id ? $t('products.editProduct') : $t('products.addProduct') }}
          </h3>
        </template>
        ...
        >
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <!-- Category -->
            <UFormField :label="$t('products.category')" name="categoryId" required>
              <USelect
            ...
            <!-- Min Stock -->
            <UFormField :label="$t('products.minStock')" name="minStock" required>
              <UInput
          ...
          <div class="flex justify-end gap-3 pt-4">
            ...
            <UButton
              ...
              color="primary"
              :loading="saving"
              :label="selectedProduct?.id ? $t('common.update') : $t('common.create')"
            />
          </div>
        </UForm>
      </UCard>
    ...
    <!-- Product View Modal -->
    <UModal v-model="showViewModal" :ui="{ width: 'sm:max-w-2xl' }">
      <UCard>
        <template #header>
          <h3 class="text-lg font-medium">
        ...
        <div v-if="viewingProduct" class="space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p class="text-gray-900 dark:text-white">{{ viewingProduct.name }}</p>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                {{ $t('products.sku') }}
              </label>
              <p class="text-gray-900 dark:text-white">{{ viewingProduct.sku }}</p>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                {{ $t('products.category') }}
              </label>
              <p class="text-gray-900 dark:text-white">{{ getCategoryName(viewingProduct.categoryId) }}</p>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                {{ $t('products.price') }}
              </label>
              <p class="text-gray-900 dark:text-white">{{ formatCurrency(viewingProduct.price) }}</p>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                {{ $t('products.stock') }}
              ...
              <p class="text-gray-900 dark:text-white">
                {{ viewingProduct.stock }} {{ getUnitSymbol(viewingProduct.unitId) }}
              </p>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
          ...
          <div v-if="viewingProduct.description">
            ...
            <p class="text-gray-900 dark:text-white">{{ viewingProduct.description }}</p>
          </div>
        </div>
...
useHead({
  ...
  meta: [
    {
      name: 'description',
      content: 'Comprehensive product management system for POS/ERP with multi-branch support'
    }
  ]
})

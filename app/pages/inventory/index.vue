...
const movementsDatePreset = ref("month");
const movementsDateRange = ref({ start: "", end: "" });
const poDatePreset = ref("month");
const poDateRange = ref({ start: "", end: "" });

// Handle date range changes
const handleMovementsDateChange = (range: { start: string; end: string }) => {
  movementsDateRange.value = range;
};

const handlePODateChange = (range: { start: string; end: string }) => {
  poDateRange.value = range;
};

// Filtered stock movements by date
const filteredMovements = computed(() => {
  let movements = [...stockMovements.value];

  if (movementsDateRange.value.start) {
    const start = new Date(movementsDateRange.value.start);
    start.setHours(0, 0, 0, 0);
    movements = movements.filter((m) => new Date(m.createdAt) >= start);
  }

  if (movementsDateRange.value.end) {
    const end = new Date(movementsDateRange.value.end);
...
// Filtered purchase orders by date
const filteredPurchaseOrders = computed(() => {
  let orders = [...purchaseOrders.value];

  if (poDateRange.value.start) {
    const start = new Date(poDateRange.value.start);
    start.setHours(0, 0, 0, 0);
    orders = orders.filter((po) => new Date(po.createdAt) >= start);
  }

  if (poDateRange.value.end) {
    const end = new Date(poDateRange.value.end);
    end.setHours(23, 59, 59, 999);
    orders = orders.filter((po) => new Date(po.createdAt) <= end);
  }
...
const handleAddStock = async (data: AddStockFormData) => {
  if (!data.productId || data.quantity <= 0) {
    toast.add({
      title: t("common.error", "Error"),
      description: t(
        "inventory.selectProductAndQuantity",
...
const handleSaveSupplier = async (data: SupplierFormData, isEdit: boolean) => {
  if (!data.name) {
    toast.add({
      title: t("common.error", "Error"),
      description: t(
        "inventory.supplierNameRequired",
...
const handleReceiveStockSuccess = async (_receipt: unknown) => {
  ...
  toast.add({
    ...
    description: t(
      "inventory.stockLotsCreated",
      "Stock lots created successfully"
    ),
    icon: "i-heroicons-check-circle",
    color: "green",
  });
...
const handleLotMove = async (_lot: StockLot) => {
  ...
  toast.add({
    ...
    description: t(
      "inventory.positionTransferComingSoon",
...
const handleLotQuarantine = async (lot: StockLot) => {
  const confirmed = confirm(
    lot.status === "quarantine"
      ? t(
          "inventory.confirmReleaseQuarantine",
...
<template>
  <div>
    ...
    <div class="mt-4 px-4 py-0.5 overflow-x-auto">
      <div class="flex lg:grid lg:grid-cols-5 gap-4 min-w-max lg:min-w-0">
        <CommonStatCard
          class="min-w-[160px] lg:min-w-0"
    ...
    <template v-if="activeTab === 'inventory'">
      ...
      <div class="lg:hidden space-y-3 px-4">
        ...
        >
          <div class="flex items-start justify-between mb-3">
            <div class="flex-1">
              ...
              <code
                class="text-xs bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded mt-1 inline-block"
                >{{ item.sku }}</code
              >
            </div>
            <UBadge
          ...
          <div class="grid grid-cols-2 gap-3 mb-3">
            <div>
              ...
              >
                {{ item.currentStock }}
                <span class="text-xs text-gray-500">{{ item.unitSymbol }}</span>
              </p>
            </div>
            <div>
              <p class="text-xs text-gray-500">{{ t("inventory.value") }}</p>
          ...
          >
            <div class="text-xs text-gray-500">
              {{ item.branchName }} • {{ t("inventory.minMax") }}:
              {{ item.minStock }}/{{ item.maxStock }}
            </div>
            <div v-if="canAdjustStock" class="flex gap-2">
              <UButton
                color="gray"
                variant="soft"
                size="sm"
        ...
        <div v-if="filteredInventory.length === 0" class="text-center py-12">
          ...
          />
          <p class="text-gray-500">{{ t("inventory.noItems") }}</p>
        </div>
      </div>
    ...
    <template v-if="activeTab === 'movements'">
      <!-- Date Range Filter -->
      <div class="px-4 py-3 bg-gray-50 dark:bg-gray-800/50 my-4">
        <CommonDateRangeSelector
          v-model="movementsDatePreset"
          :compact="true"
          @update:date-range="handleMovementsDateChange"
        />
      </div>

      <!-- Mobile Card View -->
      <div class="lg:hidden space-y-3 px-4 py-4">
        <div
        ...
        >
          ...
          <div
            class="pt-2 border-t border-gray-100 dark:border-gray-700 text-xs text-gray-500"
          >
        ...
        <div v-if="filteredMovements.length === 0" class="text-center py-12">
          <UIcon
            ...
            class="w-12 h-12 mx-auto text-gray-400 mb-4"
          />
          <p class="text-gray-500">{{ t("inventory.noMovements") }}</p>
        </div>
      </div>
      ...
      <div class="px-4 my-4 flex flex-col gap-4">
        ...
        <div class="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-3">
          <CommonDateRangeSelector
            ...
            :compact="true"
            @update:date-range="handlePODateChange"
          />
        </div>
      </div>
      ...
      <div class="lg:hidden space-y-3 px-4">
        ...
        >
          <div class="flex items-start justify-between mb-3">
            <div>
              <code
                class="text-sm bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded uppercase"
                >{{ po.id }}</code
              >
              <p class="font-semibold text-gray-900 dark:text-white mt-2">
                {{ po.supplierName }}
              </p>
              <p class="text-xs text-gray-500">{{ po.branchName }}</p>
            </div>
            <UBadge
              :color="
                po.status === 'received'
          ...
          <div class="grid grid-cols-2 gap-3 mb-3">
            <div>
              <p class="text-xs text-gray-500">{{ t("inventory.items") }}</p>
              <p class="font-medium text-gray-900 dark:text-white">
                {{ po.items.length }}
              </p>
            </div>

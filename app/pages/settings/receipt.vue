<template>
  <div class="p-6 max-w-6xl mx-auto">
    ...
    <CommonPageHeader :title="$t('settings.receipt.title')" :description="$t('settings.receipt.description')">
      <template #actions>
        <UButton color="neutral" variant="outline" :label="$t('settings.receipt.printTest')" icon="i-heroicons-printer"
          @click="printTestReceipt" />
        <UButton color="primary" :label="$t('common.save')" icon="i-heroicons-check" :loading="saving"
          @click="saveSettings" />
      </template>
    </CommonPageHeader>
          ...
          <div class="space-y-4">
            ...
            <UFormField :label="$t('settings.receipt.logo.storageType')">
              <div class="flex flex-col gap-2 mt-1">
                <label class="flex items-start gap-3 p-3 border rounded-lg cursor-pointer transition-colors" :class="settings.logoStorageType === 'local'
                  ? 'border-primary bg-primary/5'
                  : 'border-gray-200 dark:border-gray-700 hover:border-gray-300'
                  ">
                  <input type="radio" v-model="settings.logoStorageType" value="local" class="mt-0.5" />
                  <div>
                ...
                <label class="flex items-start gap-3 p-3 border rounded-lg cursor-pointer transition-colors" :class="settings.logoStorageType === 'cloudinary'
                  ? 'border-primary bg-primary/5'
                  : 'border-gray-200 dark:border-gray-700 hover:border-gray-300'
                  ">
                  <input type="radio" v-model="settings.logoStorageType" value="cloudinary" class="mt-0.5" />
                  <div>
                    <div class="font-medium flex items-center gap-2">
            ...
            <div class="flex items-center gap-4">
              ...
              <div class="flex flex-col gap-2">
                <UButton color="primary" variant="outline" :label="uploadingLogo
                  ? $t('settings.receipt.logo.uploading')
                  : $t('settings.receipt.logo.upload')
                  " :icon="uploadingLogo
                    ? 'i-heroicons-arrow-path'
                    : 'i-heroicons-arrow-up-tray'
                    " size="sm" :loading="uploadingLogo" :disabled="uploadingLogo" @click="triggerLogoUpload" />
                <UButton v-if="settings.logo" color="error" variant="ghost" :label="$t('settings.receipt.logo.remove')"
                  icon="i-heroicons-trash" size="sm" :disabled="uploadingLogo" @click="removeLogo" />
            ...
            <UCheckbox v-model="settings.showLogo" :label="$t('settings.receipt.logo.showOnReceipt')" />
          </div>
        </UCard>
        ...
        <UCard>
          ...
          <div class="space-y-4">
            ...
            <UFormField :label="$t('settings.receipt.header.customText')">
              <UTextarea v-model="settings.header.customText" :rows="2" :placeholder="$t('settings.receipt.header.customTextPlaceholder')
                " class="w-full" />
            </UFormField>
          </div>
        </UCard>
        ...
        <UCard>
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-3">
              <UCheckbox v-model="settings.content.showOrderSequence"
                :label="$t('settings.receipt.content.currentSequence')" />
              <UCheckbox v-model="settings.content.showOrderNumber"
                :label="$t('settings.receipt.content.orderNumber')" />
              <UCheckbox v-model="settings.content.showDateTime" :label="$t('settings.receipt.content.dateTime')" />
              <UCheckbox v-model="settings.content.showCashier" :label="$t('settings.receipt.content.cashier')" />
              <UCheckbox v-model="settings.content.showCustomer" :label="$t('settings.receipt.content.customer')" />
              <UCheckbox v-model="settings.content.showItemSku" :label="$t('settings.receipt.content.itemSku')" />
              <UCheckbox v-model="settings.content.showItemPrice" :label="$t('settings.receipt.content.itemPrice')" />
              <UCheckbox v-model="settings.content.showSubtotal" :label="$t('settings.receipt.content.subtotal')" />
              <UCheckbox v-model="settings.content.showDiscount" :label="$t('settings.receipt.content.discount')" />
              <UCheckbox v-model="settings.content.showTax" :label="$t('settings.receipt.content.tax')" />
              <UCheckbox v-model="settings.content.showTaxBreakdown"
                :label="$t('settings.receipt.content.taxBreakdown')" />
              <UCheckbox v-model="settings.content.showPaymentMethod"
                :label="$t('settings.receipt.content.paymentMethod')" />
              <UCheckbox v-model="settings.content.showChange" :label="$t('settings.receipt.content.change')" />
            </div>
            ...
            <div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-800">
              ...
              <div class="flex items-end gap-3">
                <UFormField :label="$t('settings.receipt.content.currentSequence') ||
                  'Current Sequence'
                  " name="orderNumber" class="flex-1">
                  <UInput v-model="currentOrderSequence" type="number" min="0" />
                </UFormField>
              </div>
              ...
              <div class="mt-3">
                <UCheckbox v-model="autoResetDailyLocal" :label="$t('settings.receipt.content.autoResetDaily') ||
                  'Auto-reset daily'
                  " />
        ...
        <UCard>
          ...
          <div class="space-y-4">
            <UFormField :label="$t('settings.receipt.footer.thankYouMessage')">
              <UInput v-model="settings.footer.thankYouMessage"
                :placeholder="$t('settings.receipt.footer.thankYouPlaceholder')" class="w-full" />
            </UFormField>

            <UFormField :label="$t('settings.receipt.footer.returnPolicy')">
              <UTextarea v-model="settings.footer.returnPolicy" :rows="2" :placeholder="$t('settings.receipt.footer.returnPolicyPlaceholder')
                " class="w-full" />
            </UFormField>

            <UFormField :label="$t('settings.receipt.footer.customText')">
              <UTextarea v-model="settings.footer.customText" :rows="2" :placeholder="$t('settings.receipt.footer.customTextPlaceholder')
                " class="w-full" />
            </UFormField>

            <div class="grid grid-cols-2 gap-3">
              <UCheckbox v-model="settings.footer.showQrCode" :label="$t('settings.receipt.footer.showQrCode')" />
              <UCheckbox v-model="settings.footer.showBarcode" :label="$t('settings.receipt.footer.showBarcode')" />
              <UCheckbox v-model="settings.footer.showSocialMedia"
                :label="$t('settings.receipt.footer.showSocialMedia')" />
              <UCheckbox v-model="settings.footer.showWebsite" :label="$t('settings.receipt.footer.showWebsite')" />
            </div>
            ...
            <!-- E-Bill URL Setting -->
            <UFormField v-if="settings.footer.showQrCode" :label="$t('settings.receipt.footer.eBillBaseUrl')">
              <UInput v-model="settings.eBillBaseUrl" type="url" :placeholder="$t('settings.receipt.footer.eBillBaseUrlPlaceholder')
                " class="w-full" />
              <template #hint>
        ...
        <UCard>
          ...
          <div class="space-y-4">
            <UFormField :label="$t('settings.receipt.paper.width')">
              <USelect v-model="settings.paper.width" :items="paperWidthOptions" class="w-full" />
            </UFormField>

            <UFormField :label="$t('settings.receipt.paper.fontSize')">
              <USelect v-model="settings.paper.fontSize" :items="fontSizeOptions" class="w-full" />
            </UFormField>

            <UFormField :label="$t('settings.receipt.paper.copies')">
              <UInput v-model.number="settings.paper.copies" type="number" min="1" max="5" class="w-full" />
            </UFormField>

            <UCheckbox v-model="settings.paper.autoPrint" :label="$t('settings.receipt.paper.autoPrint')" />
          </div>
        </UCard>
      </div>
      ...
      <div>
        <div class="sticky top-6">
          <UCard>
            ...
            <div class="flex justify-center p-4 bg-gray-100 dark:bg-gray-800 rounded-lg transition-colors">
              <!-- Receipt Paper - always white like thermal paper -->
              <div ref="receiptPreview"
                ...
                <div class="text-center border-b border-dashed border-gray-400 pb-2 mb-2">
                  <div v-if="settings.header.businessName" class="font-bold text-lg text-gray-900">
                    {{ settings.header.businessName }}
                  </div>
                  <div v-if="settings.header.address" class="text-xs text-gray-600">
                    {{ settings.header.address }}
                  </div>
                  <div v-if="settings.header.phone" class="text-xs text-gray-600">
                    {{ $t("settings.receipt.preview.tel") }}:
                    {{ settings.header.phone }}
                  </div>
                  <div v-if="settings.header.email" class="text-xs text-gray-600">
                    {{ settings.header.email }}
                  </div>
                  <div v-if="settings.header.taxId" class="text-xs text-gray-600">
                ...
                <div class="border-b border-dashed border-gray-400 pb-2 mb-2 text-xs">
                  <div v-if="settings.content.showOrderSequence" class="flex justify-between">
                    <span> No: </span>
                ...
                <!-- Items -->
                <div class="border-b border-dashed dark:border-gray-700 border-gray-400 pb-2 mb-2">
                ...
                <!-- Footer -->
                <div class="text-center text-xs border-t border-dashed border-gray-400 pt-2 text-gray-900">
                  <div v-if="settings.footer.thankYouMessage" class="font-bold mb-1 text-gray-900">
    ...
    <UCard class="mt-6">
      <template #header>
        <div class="flex items-center justify-between">
          ...
          <UButton color="primary" variant="outline" :label="$t('settings.receipt.templates.saveAsTemplate')"
            icon="i-heroicons-plus" size="sm" @click="saveAsTemplate" />
        </div>
      </template>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div v-for="template in templates" :key="template.id"
          class="border rounded-lg p-4 hover:border-primary cursor-pointer transition-colors" :class="{
          ...
          <div class="flex items-start justify-between mb-2">
            ...
            <UDropdownMenu :items="getTemplateActions(template)">
              <UButton icon="i-heroicons-ellipsis-vertical" color="neutral" variant="ghost" size="xs" @click.stop />
            </UDropdownMenu>
          </div>
        ...
        <UCard>
          <template #header>
            <div class="flex items-center justify-between">
              ...
              <UButton icon="i-heroicons-x-mark" color="neutral" variant="ghost" @click="showTemplateModal = false" />
            </div>
          </template>
          ...
          <div class="space-y-4">
            ...
            <UCheckbox v-model="newTemplateIsDefault" :label="$t('settings.receipt.templates.setAsDefault')" />
          </div>
          ...
          <template #footer>
            <div class="flex justify-end gap-2">
              ...
              <UButton color="primary" :label="$t('common.save')" @click="confirmSaveTemplate" />
            </div>
          </template>
        </UCard>
      </template>

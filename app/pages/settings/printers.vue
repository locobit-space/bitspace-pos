...
<template>
  <div class="p-6 max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
      ...
      <UButton icon="solar:add-circle-linear" :label="$t('settings.printers.addPrinter')" color="primary"
        @click="openAddModal" />
    </div>

    <!-- Info Alert -->
    <UAlert icon="solar:info-circle-linear" color="primary" variant="subtle" :title="$t('settings.printers.howItWorks')"
    ...
    <div v-if="settings.printers.length > 0">
      <div class="flex items-center justify-between mb-4">
        <!-- Search Input -->
        <div class="w-64">
          <UInput v-model="searchQuery" icon="solar:magnifer-linear" size="sm" :placeholder="$t('common.search')" />
        </div>

        <!-- View Toggle -->
        <div class="flex items-center gap-1 bg-gray-100 dark:bg-gray-800 p-1 rounded-lg">
          <UButton icon="solar:chart-square-bold-duotone" color="gray" variant="ghost" size="xs" :class="[
            viewMode === 'grid'
              ? 'bg-white dark:bg-gray-700 shadow-sm'
              : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300',
          ]" @click="viewMode = 'grid'" />
          <UButton icon="solar:list-linear" color="gray" variant="ghost" size="xs" :class="[
            viewMode === 'list'
              ? 'bg-white dark:bg-gray-700 shadow-sm'
              : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300',
          ]" @click="viewMode = 'list'" />
      ...
      <div v-if="viewMode === 'grid'" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <UCard v-for="printer in filteredPrinters" :key="printer.id"
          class="overflow-hidden transition-all hover:ring-2 hover:ring-primary-500/50">
          <div class="p-4">
            <div class="flex justify-between items-start mb-3">
              <div class="flex items-center gap-2">
                <div class="w-2 h-2 rounded-full" :class="printer.isActive ? 'bg-green-500' : 'bg-gray-300'"></div>
            ...
            <div class="space-y-2 text-sm text-gray-600 dark:text-gray-300 mb-3">
              <!-- Connection Type -->
              <div class="flex items-center gap-2">
                <UIcon :name="printer.type === 'network'
                  ? 'solar:global-linear'
                ...
                <UBadge v-if="printer.autoCut" color="green" variant="subtle" size="xs">Auto-cut</UBadge>
              </div>
              <!-- IP Address (for network types) -->
              <div v-if="
                ['network', 'wireless', 'ethernet'].includes(printer.type)
              " class="flex items-center gap-2">
                <UIcon name="solar:router-linear" class="w-4 h-4 text-gray-400" />
                <span class="font-mono text-xs">{{ printer.ip }}:{{ printer.port }}</span>
              </div>
              <!-- Paper Width -->
              <div class="flex items-center gap-2">
                <UIcon name="solar:document-text-linear" class="w-4 h-4 text-gray-400" />
                <span>{{
          ...
          <!-- Actions Footer -->
          <div class="bg-gray-50 w-full rounded-lg justify-center dark:bg-gray-800/50 px-4 py-2 flex gap-2">
            <UButton icon="i-heroicons-printer" variant="ghost" color="indigo" size="xs"
              :label="$t('settings.printers.test')" @click="testPrint(printer)" />
            <UButton icon="solar:pen-new-square-linear" variant="ghost" color="gray" size="xs"
              @click="openEditModal(printer)" />
            <UButton icon="solar:trash-bin-trash-linear" variant="ghost" color="red" size="xs"
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-800">
              <tr v-for="printer in filteredPrinters" :key="printer.id"
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center gap-2">
                    <div class="w-2.5 h-2.5 rounded-full" :class="printer.isActive ? 'bg-green-500' : 'bg-gray-300'">
                    </div>
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <span class="text-sm font-medium text-gray-900 dark:text-white">{{ printer.name }}</span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center gap-2">
                    <UIcon :name="printer.type === 'network'
                        ...
                        : printer.type === 'wireless'
                          ...
                          : printer.type === 'bluetooth'
                            ...
                            : printer.type === 'usb'
                              ...
                              : 'solar:server-path-linear'
                      " class="w-4 h-4 text-gray-400" />
                    <span class="text-sm text-gray-900 dark:text-white capitalize">{{ printer.type }}</span>
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <UBadge color="gray" variant="soft" size="xs">{{
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div v-if="['network', 'wireless', 'ethernet'].includes(printer.type)"
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                  {{ printer.paperWidth === 'custom' ? `${printer.customPaperWidth}mm` : printer.paperWidth }}
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex gap-1">
                    <UBadge v-if="printer.autoCut" color="green" variant="subtle" size="xs">Auto-cut</UBadge>
                    <UBadge v-if="printer.encoding && printer.encoding !== 'UTF-8'" color="blue" variant="subtle"
                      size="xs">{{ printer.encoding }}</UBadge>
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <div class="flex items-center justify-end gap-2">
                    <UButton icon="solar:printer-linear" variant="ghost" color="indigo" size="xs"
                      :label="$t('settings.printers.test')" @click="testPrint(printer)" />
                    <UButton icon="solar:pen-new-square-linear" variant="ghost" color="gray" size="xs"
                      @click="openEditModal(printer)" />
                    <UButton icon="solar:trash-bin-trash-linear" variant="ghost" color="red" size="xs"
    ...
    <!-- Empty State -->
    <div v-else class="text-center py-12 border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-lg">
      ...
      <UButton :label="$t('settings.printers.addPrinter')" @click="openAddModal" />
    </div>

    <PrinterFormModal v-model="isModalOpen" :printer="editingPrinter" @save="handleSave" />
  </div>
</template>

        ...
        <UCard>
          ...
          <div class="space-y-4">
            <UAlert icon="i-heroicons-exclamation-triangle" color="red" variant="subtle"
              ...
              :description="$t('settings.security.disableWarningDesc') || 'Disabling encryption will remove all encrypted data. You will need to re-enter your API keys after this action.'" />

            <UFormField :label="$t('settings.security.confirmWithPassword') || 'Confirm with your password'">
              <UInput v-model="disableEncryptionPassword" type="password"
                :placeholder="$t('settings.security.enterPassword') || 'Enter password'" class="w-full"
          ...
          <template #footer>
            <div class="flex justify-end gap-3">
              <UButton variant="ghost" @click="showDisableEncryptionModal = false; disableEncryptionPassword = ''">
                {{ $t("common.cancel") }}
              </UButton>
...
const handleDisableEncryption = async () => {
  ...
  try {
    const success = await security.disableEncryption(disableEncryptionPassword.value);
    if (success) {
      showDisableEncryptionModal.value = false;
      disableEncryptionPassword.value = "";
      toast.add({
        title: t("common.success"),
        description: t("settings.security.encryptionDisabled") || "Encryption has been disabled. Please re-enter your API keys.",
        color: "green",
      });
    } else {
      toast.add({

<template>
  <UContainer>
      ...
      <div class="space-y-4">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-muted">{{ $t('settings.tax.taxEnabledDesc') }}</p>
          </div>
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-muted">{{ $t('settings.tax.pricesIncludeTaxDesc') }}</p>
          </div>
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-muted">{{ $t('settings.tax.showTaxOnReceiptDesc') }}</p>
          </div>
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-muted">{{ $t('settings.tax.roundingMethodDesc') }}</p>
          </div>
          <USelect
          ...
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              <p v-if="rate.categories?.length" class="text-xs text-muted mt-1">
                {{ $t('settings.tax.appliesTo') }}: {{ rate.categories.join(', ') }}
              </p>
            </div>
          </div>
    ...
    <UModal v-model:open="showTaxRateModal">
      <template #header>
        <h3 class="font-semibold">
          {{ editingRate ? $t('settings.tax.editRate') : $t('settings.tax.addRate') }}
        </h3>
      </template>
    ...
    <UModal v-model:open="showDeleteModal">
      <template #header>
        <h3 class="font-semibold text-error">{{ $t('settings.tax.deleteRate') }}</h3>
      </template>

      <div class="p-4">
        <p>{{ $t('settings.tax.deleteRateConfirm', { name: rateToDelete?.name }) }}</p>
      </div>
...
const productCategories = ['Food', 'Beverages', 'Groceries', 'Electronics', 'Clothing', 'Services']

// Computed
const activeTaxRates = computed(() => taxRates.value.filter(r => r.isActive).length)
const defaultRate = computed(() => taxRates.value.find(r => r.isDefault)?.rate || 0)
...
async function deleteRate() {
  ...
  try {
    const index = taxRates.value.findIndex(r => r.id === rateToDelete.value?.id)
    if (index !== -1) {

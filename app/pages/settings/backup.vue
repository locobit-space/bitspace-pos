  ...
  <UContainer>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
      ...
      <UCard>
        ...
        <p class="text-muted mb-4">{{ $t('settings.backup.localBackupDesc') }}</p>
      ...
      <UCard>
        ...
        <p class="text-muted mb-4">{{ $t('settings.backup.cloudBackupDesc') }}</p>
        ...
        <div v-else class="space-y-4">
          <div class="flex items-center justify-center gap-3">
            ...
            <div class="text-left">
              ...
              <p class="text-sm text-muted">{{ formatSize(selectedFile.size) }}</p>
            </div>
      ...
      <div class="p-4 space-y-4">
        ...
        <div v-if="restorePreview">
          ...
          <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 space-y-1 text-sm">
            <p v-if="restorePreview.products">📦 {{ restorePreview.products }} {{ $t('products.title') }}</p>
            <p v-if="restorePreview.orders">🧾 {{ restorePreview.orders }} {{ $t('orders.title') }}</p>
            <p v-if="restorePreview.customers">👥 {{ restorePreview.customers }} {{ $t('customers.title') }}</p>
      ...
      if (data.products && Array.isArray(data.products)) {
        ...
        if (products.length > 0 && products[0]) {
          ...
          products.forEach((p: Record<string, unknown>) => {
            rows.push(Object.values(p).map(v => `"${String(v).replace(/"/g, '""')}"`).join(','))
          })
        }
      }

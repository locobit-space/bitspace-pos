...
export function useAccounting() {
  ...
  const accountMapping = computed(() =>
    getAccountMapping(settings.value.standard)
  );
  ...
  async function loadJournalEntries(): Promise<void> {
    try {
      const dbEntries = await db.journalEntries
        .orderBy("date")
        .reverse()
        .limit(100)
        .toArray(); // Load recent 100
  ...
  async function loadFromNostr(): Promise<void> {
    ...
    const accountEvents = await nostrData.getAllEventsOfKind<any>(
      NOSTR_KINDS.ACCOUNT
    );
    ...
    const journalEvents = await nostrData.getAllEventsOfKind<any>(
  ...
  async function updateSettings(
    newSettings: Partial<AccountingSettings>
  ): Promise<void> {
  ...
  function generateEntryNumber(): string {
    ...
    const count =
      journalEntries.value.filter((e) => e.entryNumber.includes(`JE-${year}`))
        .length + 1;
   ...
   */
  async function createSalesJournalEntry(
    order: Order
  ): Promise<JournalEntry | null> {
  ...
  async function createStockAdjustmentEntry(
    ...
    return createJournalEntry(`Stock Adjustment: ${productName}`, lines, {
  ...
  async function recognizeMembershipRevenue(
    ...
    return createJournalEntry(`Membership Recognition: ${memberName}`, lines, {
      sourceType: "membership",
      sourceId: membershipId,
    });
  }
  ...
  async function createExpenseEntry(
    ...
    return createJournalEntry(description, lines, {
      sourceType: "expense",
      sourceId: expenseId,
    });
  }
   ...
   * Categories: sales, tips, investment, refund, deposit, other
   */
  async function createIncomeEntry(
    description: string,
    amount: number,
    ...
    // Map income category to revenue account
    // For simplicity, use main sales revenue account
    // Could extend with more specific accounts per category
    const revenueAccount = mapping.salesRevenue;

    const cashAccountInfo = getAccount(cashAccount);
    const revenueAccountInfo = getAccount(revenueAccount);

    const lines: Omit<JournalEntryLine, "id">[] = [
      // Debit: Cash/Bank/Lightning (asset increases)
      {
        accountCode: cashAccount,
        accountName: cashAccountInfo?.name || "Cash",
      ...
      // Credit: Revenue (revenue increases)
      {
        ...
        accountName: revenueAccountInfo?.name || "Revenue",
    ...
    return createJournalEntry(`Income: ${description}`, lines, {
      sourceType: "income",
      sourceId: incomeId,
    });
  }

...
export function useStaffAuth() {
   ...
   */
  async function signAuthChallenge(nsec: string, challenge: AuthChallenge): Promise<string | null> {
    try {
      // Decode nsec to hex
   ...
   */
  function getNpubFromNsec(nsec: string): { npub: string; pubkeyHex: string } | null {
    try {
      const privateKeyHex = nostrKey.decodePrivateKey(nsec);
      const pubkeyHex = getPublicKey(hexToBytes(privateKeyHex));
  ...
  async function loginWithNostr(
    ...
    try {
      ...
      if (user.revokedAt) {
        return { success: false, error: `Access revoked: ${user.revocationReason || 'Contact administrator'}` };
      }
      
      // Check expiry
      if (user.expiresAt && new Date(user.expiresAt) < new Date()) {
        return { success: false, error: 'Access has expired. Contact administrator.' };
      }
      ...
      if (!isValid) {
        ...
        if (attempts >= MAX_LOGIN_ATTEMPTS) {
          return { 
            ...
            user: { ...user, failedLoginAttempts: attempts, lockedUntil: new Date(Date.now() + LOCKOUT_DURATION).toISOString() } as StoreUser
          };
        }
        return { 
          success: false, 
  ...
  function extendSession(): void {
    if (authSession.value && authSession.value.isValid) {
      authSession.value.expiresAt = new Date(Date.now() + SESSION_DURATION).toISOString();
    }
  }

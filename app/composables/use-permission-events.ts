...
import { getPublicKey, finalizeEvent, utils } from "nostr-tools";
...
export function usePermissionEvents() {
  ...
  function verifyPermissionGrant(grant: PermissionGrant): boolean {
    try {
      ...
      const revocation = permissionRevocations.value.find(
        (r) =>
          r.grantId === grant.id &&
          new Date(r.createdAt) > new Date(grant.createdAt),
      );
      if (revocation) {
  ...
  async function saveRevocations(): Promise<void> {
    await security.encryptAndStore(
      REVOCATIONS_STORAGE_KEY,
      permissionRevocations.value,
    );
  }
  ...
  async function loadPermissionData(): Promise<void> {
    const grants =
      await security.retrieveAndDecrypt<PermissionGrant[]>(GRANTS_STORAGE_KEY);
    if (grants) {
      permissionGrants.value = grants;
    }

    const revocations = await security.retrieveAndDecrypt<
      PermissionRevocation[]
    >(REVOCATIONS_STORAGE_KEY);
    if (revocations) {
   ...
   */
  async function publishGrantToRelays(
   ...
   */
  async function fetchGrantsFromRelays(
    _npub: string,

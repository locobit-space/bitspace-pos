...
export function useExpenses() {
    ...
    const lastMonth = expenses.value
      .filter(
        (e) =>
          ...
          new Date(e.date) < thisMonthStart
      )
      .reduce((sum, e) => sum + e.amount, 0);
  ...
  const expensesByCategory = computed(() => {
    ...
    return Array.from(categoryTotals.entries())
      .map(([name, value]) => ({
        ...
        value,
        color:
          EXPENSE_CATEGORIES.find((c) => c.value === name)?.color || "#9ca3af",
      }))
  ...
  async function createExpense(
    data: Omit<Expense, "id" | "createdAt" | "updatedAt" | "synced">
    ...
    try {
      ...
      try {
        ...
        const defaultAccountCode =
          accounting.currentStandard.value === "lao" ? "5100" : "60100";

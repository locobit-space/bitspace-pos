...
export const useCurrency = () => {
  ...
  const refreshRates = async () => {
    ...
    try {
      ...
      btcPriceLAK.value = btcPrices.LAK || btcPrice.value * (fiatRates.lak || 20500);
      btcPriceTHB.value = btcPrices.THB || btcPrice.value * (fiatRates.thb || 35);
      ...
      // SATS to all currencies (1 sat = BTC / 100_000_000)
      const satsToUsd = btcPrice.value / 100_000_000;
      const satsToLak = btcPriceLAK.value / 100_000_000;
      const satsToThb = btcPriceTHB.value / 100_000_000;

      setRate('SATS', 'USD', satsToUsd, 'api');
      setRate('SATS', 'LAK', satsToLak, 'api');
      setRate('SATS', 'THB', satsToThb, 'api');
      setRate('SATS', 'BTC', 1 / 100_000_000, 'manual');
  ...
  const setFallbackRates = () => {
    ...
    setRate('BTC', 'SATS', 100_000_000, 'manual');
    ...
    const satsToLak = btcPriceLAK.value / 100_000_000;
    const satsToThb = btcPriceTHB.value / 100_000_000;
    const satsToUsd = fallbackBtcUsd / 100_000_000;
  ...
  const getRate = (from: CurrencyCode, to: CurrencyCode): number => {
    ...
    console.warn(`No rate found for ${from} to ${to}, using fallback calculation`);
    
    // Last resort: calculate through BTC
    const fromBtc = exchangeRates.value.get(`${from}-BTC`)?.rate || 

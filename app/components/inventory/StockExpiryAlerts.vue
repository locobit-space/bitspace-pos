  ...
  <div class="space-y-4">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
      ...
      <div 
        ...
        :class="selectedFilter === 'expired' 
          ? 'bg-red-500 text-white shadow-lg shadow-red-500/25' 
          : 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400'"
        @click="selectedFilter = selectedFilter === 'expired' ? 'all' : 'expired'"
      >
        <div class="flex items-center gap-3">
          <span class="text-2xl">🚫</span>
          <div>
            <p class="text-2xl font-bold">{{ expiredCount }}</p>
            <p class="text-xs opacity-75">{{ $t('inventory.expired') || 'Expired' }}</p>
          </div>
        </div>
      </div>
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs opacity-75">{{ $t('inventory.urgentExpiry') || '≤3 days' }}</p>
          </div>
        </div>
      </div>
      ...
      <div 
        class="p-4 rounded-2xl cursor-pointer transition-all hover:scale-[1.02]"
        :class="selectedFilter === 'critical' 
          ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/25' 
          : 'bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400'"
        @click="selectedFilter = selectedFilter === 'critical' ? 'all' : 'critical'"
      >
        <div class="flex items-center gap-3">
          <span class="text-2xl">⚠️</span>
          <div>
            <p class="text-2xl font-bold">{{ criticalCount }}</p>
            <p class="text-xs opacity-75">{{ $t('inventory.criticalExpiry') || '≤7 days' }}</p>
          </div>
        </div>
      </div>
      ...
      <div 
        class="p-4 rounded-2xl cursor-pointer transition-all hover:scale-[1.02]"
        :class="selectedFilter === 'warning' 
          ? 'bg-yellow-500 text-white shadow-lg shadow-yellow-500/25' 
          : 'bg-yellow-50 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-400'"
        @click="selectedFilter = selectedFilter === 'warning' ? 'all' : 'warning'"
      >
        <div class="flex items-center gap-3">
          <span class="text-2xl">⏰</span>
          <div>
            <p class="text-2xl font-bold">{{ warningCount }}</p>
            <p class="text-xs opacity-75">{{ $t('inventory.warningExpiry') || '≤30 days' }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
      ...
      <div class="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
        <h3 class="font-semibold text-gray-900 dark:text-white flex items-center gap-2">
          ...
          <UBadge v-if="filteredAlerts.length > 0" :color="getBadgeColor" variant="subtle">
      ...
      <div class="max-h-[400px] overflow-y-auto">
        ...
        >
          ...
          <div class="flex items-center gap-1 shrink-0">
            ...
            <UTooltip v-if="!alert.acknowledged" :text="$t('inventory.acknowledge') || 'Acknowledge'">
              <UButton
        ...
        >
          ...
          <p class="font-medium">
            {{ selectedFilter === 'all' 
              ...
              : ($t('inventory.noAlertsInCategory') || 'No alerts in this category') }}
          </p>
          <p class="text-sm">
            {{ $t('inventory.stockHealthy') || 'All stock is within acceptable expiry dates' }}
          </p>
        </div>
      </div>
    </div>
...
// State
const selectedFilter = ref<'all' | 'expired' | 'urgent' | 'critical' | 'warning'>('all');
...
const filteredAlerts = computed(() => {
  ...
  return expiryAlerts.value.filter(a => a.alertLevel === selectedFilter.value);
});
...
function getAlertBgClass(level: string): string {
  switch (level) {
    ...
    case 'urgent': return 'bg-orange-100 dark:bg-orange-900/30';
    case 'critical': return 'bg-amber-100 dark:bg-amber-900/30';
...
function getAlertTextClass(level: string): string {
  switch (level) {
    ...
    case 'critical': return 'text-amber-600 dark:text-amber-400';
    case 'warning': return 'text-yellow-600 dark:text-yellow-400';
    default: return 'text-gray-600 dark:text-gray-400';
  }
}

function getLotStatusColor(status: string): 'green' | 'yellow' | 'red' | 'orange' | 'gray' {
  switch (status) {
...
async function acknowledgeAll() {
  ...
  toast.add({
    title: t('common.success') || 'Success',
    description: t('inventory.allAlertsAcknowledged') || 'All alerts acknowledged',
    icon: 'i-heroicons-check-circle',
    color: 'green',
  });
}

...
const tenure = computed(() => {
    ...
    if (years > 0) return `${years} ${t("common.years")}, ${months} ${t("common.months")}`;
        ...
        <div class="relative bg-gradient-to-r from-primary-500 to-primary-600 p-6 text-white">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-primary-100">{{ employee.position || t("employees.noPosition") }}</p>
                    <div class="flex items-center gap-2 mt-2">
                        <UBadge :color="getStatusColor(employee.status)" size="sm">
                            {{ t(`employees.status.${employee.status}`) }}
                        </UBadge>
                        <span class="text-sm text-primary-200">{{ employee.employeeCode }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div class="bg-gray-50 dark:bg-gray-900 rounded-lg p-3 text-center">
                    ...
                    <p class="text-xs text-gray-500">{{ t(`employees.salaryType.${employee.salaryType}`) }}</p>
                </div>
                <div class="bg-gray-50 dark:bg-gray-900 rounded-lg p-3 text-center">
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">
                ...
                <div class="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                        ...
                        <p class="font-medium text-gray-900 dark:text-white">{{
                            t(`employees.type.${employee.employmentType}`) }}</p>
                    </div>
                    <div>
            ...
            <div v-if="employee.emergencyContactName" class="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
                ...
                <div class="grid grid-cols-3 gap-4 text-sm">
                    <div>
                        ...
                        <p class="font-medium text-gray-900 dark:text-white">{{ employee.emergencyContactName }}</p>
                    </div>
                    <div>
                        <p class="text-gray-500">{{ t("employees.contactPhone") }}</p>
                        <p class="font-medium text-gray-900 dark:text-white">{{ employee.emergencyContactPhone }}</p>
                    </div>
                    <div>
                        <p class="text-gray-500">{{ t("employees.contactRelation") }}</p>
                        <p class="font-medium text-gray-900 dark:text-white">{{ employee.emergencyContactRelation }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
                ...
                <div class="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div v-if="employee.preferredPaymentMethod === 'lightning'">
                        ...
                        <p class="font-medium text-gray-900 dark:text-white">{{ employee.lightningAddress }}</p>
                    </div>
                    <div v-if="employee.commissionEnabled">
                        <p class="text-gray-500">{{ t("employees.commissionRate") }}</p>
                        <p class="font-medium text-gray-900 dark:text-white">{{ employee.commissionRate }}%</p>
                    </div>
                </div>
            </div>
            ...
            <div v-if="employee.notes" class="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
                <h3 class="font-medium text-gray-900 dark:text-white mb-2">{{ t("employees.notes") }}</h3>

...
import type { Employee, EmploymentType, SalaryType, EmployeePaymentMethod, EmployeeStatus } from "~/types";
...
const form = ref({
    ...
    employmentType: props.employee?.employmentType || "full-time" as EmploymentType,
    ...
    preferredPaymentMethod: props.employee?.preferredPaymentMethod || "cash" as EmployeePaymentMethod,
...
const paymentMethodOptions = [
    { value: "cash", label: t("employees.paymentMethod.cash"), icon: "i-heroicons-banknotes" },
    { value: "bank", label: t("employees.paymentMethod.bank"), icon: "i-heroicons-building-library" },
    { value: "lightning", label: t("employees.paymentMethod.lightning"), icon: "i-heroicons-bolt" },
];
...
const tabs = [
    ...
    { id: "employment", label: t("employees.tabs.employment"), icon: "i-heroicons-briefcase" },
    { id: "compensation", label: t("employees.tabs.compensation"), icon: "i-heroicons-banknotes" },
...
async function handleSubmit() {
    ...
    try {
        ...
        if (isEditMode.value && props.employee) {
            savedEmployee = await employeesStore.updateEmployee(props.employee.id, form.value) as Employee;
        } else {
            savedEmployee = await employeesStore.addEmployee(form.value as Omit<Employee, "id" | "employeeCode" | "createdAt" | "updatedAt">);
        }
...
<template>
    <div class="bg-white dark:bg-gray-800 rounded-xl max-h-[90vh] overflow-hidden flex flex-col">
        <!-- Header -->
        <div class="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                <UIcon :name="isEditMode ? 'i-heroicons-pencil-square' : 'i-heroicons-user-plus'"
        ...
        <div class="flex-1 overflow-y-auto p-4">
            ...
            <div v-show="activeTab === 'basic'" class="space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <UFormField :label="t('employees.firstName')" required>
                        <UInput v-model="form.firstName" :placeholder="t('employees.firstNamePlaceholder')" />
                    </UFormField>
                    <UFormField :label="t('employees.lastName')" required>
                        <UInput v-model="form.lastName" :placeholder="t('employees.lastNamePlaceholder')" />
                    </UFormField>
                </div>

                <UFormField :label="t('employees.displayName')">
                    <UInput v-model="form.displayName" :placeholder="t('employees.displayNamePlaceholder')" />
                </UFormField>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <UFormField :label="t('employees.gender.label')">
                        <USelect v-model="form.gender" :items="genderOptions" value-key="value" label-key="label" />
                    </UFormField>
                </div>

                <UFormField :label="t('employees.nationalId')">
                    <UInput v-model="form.nationalId" :placeholder="t('employees.nationalIdPlaceholder')" />
                </UFormField>
                ...
                <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <UFormField :label="t('employees.contactName')">
                            <UInput v-model="form.emergencyContactName" />
                        </UFormField>
                        <UFormField :label="t('employees.contactPhone')">
                            <UInput v-model="form.emergencyContactPhone" />
                        </UFormField>
                        <UFormField :label="t('employees.contactRelation')">
                            <UInput v-model="form.emergencyContactRelation" />
                        </UFormField>
                    </div>
                </div>
            </div>
            ...
            <div v-show="activeTab === 'employment'" class="space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <UFormField :label="t('employees.department')">
                        <UInput v-model="form.department" :placeholder="t('employees.departmentPlaceholder')" />
                    </UFormField>
                    <UFormField :label="t('employees.position')">
                        <UInput v-model="form.position" :placeholder="t('employees.positionPlaceholder')" />
                    </UFormField>
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <UFormField :label="t('employees.employmentType')">
                        <USelect v-model="form.employmentType" :items="employmentTypeOptions" value-key="value"
                            label-key="label" />
                    </UFormField>
                ...
                <UFormField :label="t('employees.notes')">
                    <UTextarea v-model="form.notes" :rows="3" :placeholder="t('employees.notesPlaceholder')" />
                </UFormField>
            </div>
            ...
            <div v-show="activeTab === 'compensation'" class="space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <UFormField :label="t('employees.salaryType.label')">
                        <USelect v-model="form.salaryType" :items="salaryTypeOptions" value-key="value"
                            label-key="label" />
                    </UFormField>
                    <UFormField :label="t('employees.currency')">
                        <USelect v-model="form.currency" :items="currencyOptions" value-key="value" label-key="label" />
                    </UFormField>
                </div>
                ...
                <UFormField :label="t('employees.overtimeRate')">
                    <UInput v-model.number="form.overtimeRate" type="number" min="1" step="0.1">
                        <template #trailing>
                ...
                <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <UFormField :label="t('employees.preferredPayment')">
                        <USelect v-model="form.preferredPaymentMethod" :items="paymentMethodOptions" value-key="value"
                            label-key="label" />
                    </UFormField>

                    <div v-if="form.preferredPaymentMethod === 'bank'"
                    ...
                    <div v-if="form.preferredPaymentMethod === 'lightning'" class="mt-4">
                        <UFormField :label="t('employees.lightningAddress')">
                            <UInput v-model="form.lightningAddress" placeholder="user@wallet.com"
                ...
                <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <div v-if="form.commissionEnabled">
                        <UFormField :label="t('employees.commissionRate')">
                            <UInput v-model.number="form.commissionRate" type="number" min="0" max="100" step="0.5">
                                <template #trailing>
                ...
                <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        <UFormField :label="t('employees.annualLeave')">
                            <UInput v-model.number="form.annualLeaveBalance" type="number" min="0" />
                        </UFormField>
                        <UFormField :label="t('employees.sickLeave')">
                            <UInput v-model.number="form.sickLeaveBalance" type="number" min="0" />
                        </UFormField>
                        <UFormField :label="t('employees.personalLeave')">
                            <UInput v-model.number="form.personalLeaveBalance" type="number" min="0" />
                        </UFormField>
                    </div>
                </div>
            </div>
            ...
            <div v-show="activeTab === 'access'" class="space-y-4">
                <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-900 rounded-lg">
                    <div>
                        <p class="font-medium text-gray-900 dark:text-white">{{ t("employees.posAccess") }}</p>
                        <p class="text-sm text-gray-500">{{ t("employees.posAccessDescription") }}</p>
                    </div>
                ...
                <div v-if="form.canAccessPOS">
                    <UFormField :label="t('employees.pin')">
                        <UInput v-model="form.pin" type="password" maxlength="6"
                            :placeholder="t('employees.pinPlaceholder')" icon="i-heroicons-lock-closed" />
                        <template #hint>
                            <span class="text-xs text-gray-500">{{ t("employees.pinHint") }}</span>
                        </template>
                    </UFormField>
                </div>
            </div>
        ...
        <div
            ...
            <UButton color="primary" :loading="isLoading" :disabled="!isValid" @click="handleSubmit">

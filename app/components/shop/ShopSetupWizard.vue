...
const allSteps = computed<WizardStep[]>(() => {
  ...
  if (props.showBranch) {
    steps.push({
      ...
      isValid: () => branchForm.name.trim().length > 0 && branchForm.code.trim().length > 0,
    });
  }
...
const completeSetup = async () => {
  ...
  try {
    ...
    if (shopForm.visibility === 'public') {
      let hasNostrAuth: string | boolean | undefined = nostrUser.user.value?.publicKey;
...
// ============ Watchers ============
watch(() => branchForm.name, (name) => {
  if (!branchForm.code) {
    branchForm.code = name.trim().toUpperCase().replace(/\s+/g, '-').slice(0, 8);
  }
});
...
onMounted(() => {
  ...
  if (props.mode === 'workspace') {
    ...
    console.log('[ShopSetup] Generated new company code for workspace:', generatedCompanyCode.value);
  } else if (!company.hasCompanyCode.value) {
    // Initial setup without existing code
    generatedCompanyCode.value = company.generateCompanyCode();
    console.log('[ShopSetup] Generated new company code for initial setup:', generatedCompanyCode.value);
  } else {
...
<template>
  <div class="min-h-dvh bg-gray-50 dark:bg-gray-900 flex flex-col">
    <!-- Mobile Header -->
    <div class="sticky top-0 z-20 bg-white/95 dark:bg-gray-800/95 backdrop-blur-lg border-b border-gray-200 dark:border-gray-700 safe-area-inset-top">
      <div class="flex items-center justify-between px-4 py-3">
        <button
          ...
          @click="prevStep"
        >
          <Icon name="i-heroicons-arrow-left" size="24" class="text-gray-600 dark:text-gray-400" />
        </button>
        <button
          v-else
          class="p-2 -ml-2 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 active:scale-95 transition-all"
          @click="emit('cancel')"
        >
          <Icon name="i-heroicons-x-mark" size="24" class="text-gray-600 dark:text-gray-400" />
        </button>
      ...
      <div class="max-w-lg mx-auto px-4 py-6 pb-32">
        ...
        <div v-show="currentStepData?.id === 'type'" class="space-y-4">
          <p class="text-center text-gray-600 dark:text-gray-400 mb-6">
            {{ $t('shop.setup.selectTypeDesc', 'Choose what best describes your business') }}
          </p>
          ...
          <div class="grid grid-cols-2 gap-3">
            <button
              ...
              :class="{
                'border-primary-500 bg-primary-50 dark:bg-primary-900/20 shadow-lg shadow-primary-500/20': shopType === type.id,
                'border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600': shopType !== type.id,
              }"
        ...
        <div v-show="currentStepData?.id === 'info'" class="space-y-4">
          ...
          >
            ...
            <div class="min-w-0 flex-1">
              <p class="text-sm font-medium text-gray-900 dark:text-white">{{ selectedShopType.label }}</p>
              <p class="text-xs text-gray-500 truncate">{{ selectedShopType.description }}</p>
            </div>
            <button
          ...
          <UFormField :label="$t('shop.address', 'Address')" :hint="$t('common.optional', 'optional')">
            <UTextarea
              v-model="shopForm.address"
              :placeholder="$t('shop.addressPlaceholder', 'Enter your shop address')"
              :rows="2"
              class="w-full"
            />
          </UFormField>

          <UFormField :label="$t('shop.phone', 'Phone')" :hint="$t('common.optional', 'optional')">
            <UInput
          ...
          <div v-if="mode === 'initial'" class="pt-4">
            ...
            <div class="grid grid-cols-2 gap-3">
              <button
                ...
                :class="{
                  'border-primary-500 bg-primary-50 dark:bg-primary-900/20': shopForm.visibility === 'private',
                  'border-gray-200 dark:border-gray-700': shopForm.visibility !== 'private',
                }"
                @click="shopForm.visibility = 'private'"
              >
                <Icon name="i-heroicons-lock-closed" size="24" class="mb-2" :class="shopForm.visibility === 'private' ? 'text-primary-600' : 'text-gray-400'" />
              ...
              <button
                ...
                class="p-4 rounded-xl border-2 text-left transition-all active:scale-[0.98]"
                :class="{
                  'border-primary-500 bg-primary-50 dark:bg-primary-900/20': shopForm.visibility === 'public',
                  'border-gray-200 dark:border-gray-700': shopForm.visibility !== 'public',
                }"
                @click="shopForm.visibility = 'public'"
              >
                <Icon name="i-heroicons-globe-alt" size="24" class="mb-2" :class="shopForm.visibility === 'public' ? 'text-primary-600' : 'text-gray-400'" />
        ...
        <div v-show="currentStepData?.id === 'branch'" class="space-y-4">
          <div class="text-center mb-6">
            <div class="w-16 h-16 mx-auto mb-3 rounded-2xl bg-green-100 dark:bg-green-900/30 flex items-center justify-center">
              <Icon name="i-heroicons-map-pin" size="32" class="text-green-600 dark:text-green-400" />
            </div>
          ...
          <UFormField :label="$t('branch.code', 'Branch Code')" required>
            ...
            />
            <template #hint>
              <span class="text-xs text-gray-500">{{ $t('branch.codeHint', 'Short code for this branch') }}</span>
            </template>
          </UFormField>
        ...
        <div v-show="currentStepData?.id === 'settings'" class="space-y-4">
          ...
          <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-xl">
            <div>
              ...
              <p class="text-xs text-gray-500">
                {{ $t('settings.general.enableTipsDescription', 'Allow customers to tip') }}
              </p>
            </div>
            <USwitch v-model="settingsForm.tipEnabled" />
          </div>

          <UFormField :label="$t('settings.general.receiptFooter', 'Receipt Footer')">
            <UTextarea
              v-model="settingsForm.receiptFooter"
              :placeholder="$t('settings.general.receiptFooterPlaceholder', 'Thank you!')"
              :rows="2"
              class="w-full"
            />
          </UFormField>
          ...
          <!-- Product Templates -->
          <div class="pt-4">
            <ShopProductTemplatePreview v-model="applyTemplates" :shop-type="shopType" />
          </div>
        </div>
        ...
        <div v-show="currentStepData?.id === 'marketplace'" class="space-y-4">
          <div class="text-center mb-6">
            <div class="w-16 h-16 mx-auto mb-3 rounded-2xl bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center">
              <Icon name="i-heroicons-globe-alt" size="32" class="text-blue-600 dark:text-blue-400" />
            </div>
            <p class="text-gray-600 dark:text-gray-400 text-sm">
          ...
          <UFormField :label="$t('shop.marketplace.description', 'Description')">
            <UTextarea
              ...
              :placeholder="$t('shop.marketplace.descriptionPlaceholder', 'Tell customers about your store...')"
              :rows="3"
              class="w-full"
            />
          </UFormField>
          ...
          <UFormField :label="$t('shop.marketplace.services', 'Services')">
            <div class="flex flex-wrap gap-2">
              <button
                ...
                :class="{
                  'border-primary-500 bg-primary-50 dark:bg-primary-900/20': marketplaceForm.services.includes(option.value),
                  'border-gray-200 dark:border-gray-700': !marketplaceForm.services.includes(option.value),
                }"
                @click="
              ...
              >
                ...
                <span class="text-gray-700 dark:text-gray-300">{{ option.name }}</span>
              </button>
            </div>
          </UFormField>
          ...
          <div class="space-y-3 pt-2">
            ...
            <div class="flex items-center justify-between p-3 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-200 dark:border-amber-800">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <p class="font-medium text-gray-900 dark:text-white text-sm">Lightning</p>
            ...
            <div class="flex items-center justify-between p-3 bg-orange-50 dark:bg-orange-900/20 rounded-xl border border-orange-200 dark:border-orange-800">
              <div class="flex items-center gap-3">
                <span class="text-2xl">₿</span>
                <div>
                  <p class="font-medium text-gray-900 dark:text-white text-sm">Bitcoin</p>
        ...
        <div v-show="currentStepData?.id === 'confirm'" class="space-y-4">
          ...
          <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-5 shadow-sm">
            ...
            <div class="space-y-2 border-t border-gray-100 dark:border-gray-700 pt-4">
              <div v-if="shopForm.address" class="flex items-start gap-3">
                <Icon name="i-heroicons-map-pin" size="18" class="text-gray-400 mt-0.5 shrink-0" />
                <span class="text-sm text-gray-600 dark:text-gray-300">{{ shopForm.address }}</span>
              </div>
              <div v-if="shopForm.phone" class="flex items-center gap-3">
                <Icon name="i-heroicons-phone" size="18" class="text-gray-400 shrink-0" />
                <span class="text-sm text-gray-600 dark:text-gray-300">{{ shopForm.phone }}</span>
              </div>
              <div v-if="branchForm.name" class="flex items-center gap-3">
                <Icon name="i-heroicons-building-office" size="18" class="text-gray-400 shrink-0" />
                <span class="text-sm text-gray-600 dark:text-gray-300">{{ branchForm.name }} ({{ branchForm.code }})</span>
              </div>
            </div>
          </div>
          ...
          >
            <div class="flex items-center gap-2 mb-2">
              <Icon name="i-heroicons-qr-code" size="20" class="text-primary-600" />
              <p class="text-sm font-medium text-primary-900 dark:text-primary-100">
                {{ $t('auth.company.connectTitle', 'Company Code') }}
              </p>
            </div>
            <p class="font-mono text-2xl font-bold tracking-widest text-primary-700 dark:text-primary-300">
              {{ generatedCompanyCode }}
            </p>
            <p class="text-xs text-primary-600 dark:text-primary-400 mt-2">
              {{ $t('shop.setup.companyCodeHint', 'Share with staff to connect their devices') }}
            </p>
          </div>
            ...
            <div class="text-sm">
              ...
              <ul class="space-y-1 text-blue-700 dark:text-blue-300">
                <li class="flex items-center gap-2">
                  <Icon name="i-heroicons-check" size="14" class="text-blue-500" />
                  {{ $t('shop.setup.step1.description', 'Your shop will be created locally') }}
                </li>
                <li class="flex items-center gap-2">
                  <Icon name="i-heroicons-check" size="14" class="text-blue-500" />
                  {{ $t('shop.setup.step2.description', 'Data will sync to Nostr relays') }}
                </li>
                <li class="flex items-center gap-2">
                  <Icon name="i-heroicons-check" size="14" class="text-blue-500" />
                  {{ $t('shop.setup.step3.description', 'You can switch between workspaces anytime') }}
                </li>
              </ul>
            </div>
          </div>
    ...
    <!-- Bottom Action Bar -->
    <div class="fixed bottom-0 inset-x-0 bg-white/95 dark:bg-gray-800/95 backdrop-blur-lg border-t border-gray-200 dark:border-gray-700 safe-area-inset-bottom">

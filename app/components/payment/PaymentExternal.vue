...
<template>
  <div class="text-center max-h-[70vh] overflow-y-auto">
    ...
    <div v-else-if="step === 'confirm'" class="space-y-4">
      ...
      <!-- Selected Provider -->
      <div v-if="selectedProvider" class="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
        <div class="flex items-center justify-center gap-3">
          ...
          <div class="text-left">
            ...
            <p class="text-sm text-gray-500">{{ selectedProvider.description }}</p>
          </div>
        </div>
      </div>

      <!-- Amount Confirmation -->
      <div class="p-4 bg-primary-50 dark:bg-primary-500/10 border border-primary-200 dark:border-primary-500/20 rounded-xl">
        <p class="text-sm text-primary-600 dark:text-primary-400 mb-1">{{ t('payment.external.collectAmount') }}</p>
      ...
      <div class="text-left">
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
          {{ t('payment.external.reference') }} <span class="text-gray-400">({{ t('common.optional') }})</span>
        </label>
        <UInput
    ...
    <div v-else-if="step === 'complete'" class="py-8">
      ...
      <div v-if="selectedProvider" class="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-800 rounded-full">

...
const isLightningConfigured = computed(() => lightning.settings.value?.isConfigured);
...
const paymentMethods = computed(() => [
  {
    ...
    badge: lightningProvider.value === 'lnurl' ? 'Lightning Address' : lightningProvider.value?.toUpperCase(),
  },
  {
  ...
  },
  {
    ...
    icon: '📱',
    available: isLightningConfigured.value && !!lightning.settings.value?.bolt12Offer,
    badge: 'BOLT12',
  },
]);
  ...
  <div class="min-h-[300px]">
    ...
    <div v-if="showMethodSelect" class="space-y-4">
      ...
      <div class="text-center mb-6">
        ...
        <div class="mt-2 p-3 bg-gray-100 dark:bg-gray-800 rounded-xl inline-block">
      ...
      <div class="space-y-3">
        ...
        >
          <div class="w-14 h-14 rounded-xl bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-3xl group-hover:scale-110 transition-transform">
            {{ method.icon }}
          </div>
          <div class="flex-1">
            <div class="flex items-center gap-2">
              <span class="font-semibold text-gray-900 dark:text-white">{{ method.name }}</span>
              <UBadge v-if="method.badge" color="primary" variant="soft" size="xs">
                {{ method.badge }}
              </UBadge>
            </div>
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ method.description }}</p>
            <p v-if="!method.available && method.id === 'lightning'" class="text-xs text-amber-600 dark:text-amber-400 mt-1">
      ...
      <div v-if="!isLightningConfigured" class="mt-6 p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl text-center">
    ...
    <div v-else>
      ...
      <!-- Static QR Payment -->
      <div v-else-if="selectedMethod === 'qr_static'" class="text-center space-y-4">
        <PaymentStaticQR

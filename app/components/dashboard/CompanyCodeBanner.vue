...
<template>
    <UCard v-if="!hasCompanyCode && isVisible" class="border-l-4 border-l-primary-500">
        <div class="flex items-start gap-4">
            <div class="p-2 rounded-lg bg-primary/10">
                <UIcon name="i-heroicons-building-office-2" class="w-8 h-8 text-primary" />
            </div>
            ...
            <div class="flex-1">
                ...
                <p class="text-sm text-gray-500 mb-4">
                    {{ t("auth.company.connectDescription") || "Enter your company's 6-digit code to sync data" }}
                </p>

                <div class="flex flex-col sm:flex-row gap-3">
                    <UInput v-model="codeInput" placeholder="XXXX-XXXX-XXXX" icon="i-heroicons-key" size="lg"
                        maxlength="14" class="w-full sm:w-52 font-mono tracking-wider" @input="formatCodeInput"
                        @keyup.enter="handleConnect" />
                    <UButton size="lg" :loading="isConnecting" :disabled="!isValidCode" @click="handleConnect">
            ...
            <!-- Dismiss -->
            <UButton variant="ghost" color="gray" icon="i-heroicons-x-mark" size="sm" @click="dismiss" />
        </div>
    </UCard>
</template>
...
async function handleConnect() {
    ...
    try {
        ...
        try {
            ...
            console.log("[CompanyCode] Discovered owner:", ownerPubkey?.slice(0, 8) + "...");
        } catch (e) {
        ...
        try {
            ...
            console.warn("[CompanyCode] Sync failed, will retry on page load:", syncError);
        }
        ...
        errorMsg.value = t("auth.company.connectError") || "Failed to connect. Please check the code.";
    } finally {
...
onMounted(() => {
    if (import.meta.client) {
        isDismissed.value = sessionStorage.getItem("company_banner_dismissed") === "true";
    }
});
</script>

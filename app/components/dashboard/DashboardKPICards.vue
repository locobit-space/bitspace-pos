...
<template>
  <div class="grid grid-cols-12 gap-3">
    ...
    <div class="col-span-12 sm:col-span-6 lg:col-span-3">
      ...
      >
        <div class="flex items-center justify-between mb-2">
          ...
          >
            {{ todayStats.revenueChange >= 0 ? "+" : ""
            }}{{ todayStats.revenueChange.toFixed(1) }}%
          </span>
        </div>
        ...
        <p
          class="text-xl font-semibold text-gray-900 dark:text-white tabular-nums"
        >
    ...
    <div class="col-span-12 sm:col-span-6 lg:col-span-3">
      <div
        class="h-full bg-amber-50 dark:bg-amber-900/10 rounded-lg p-4 border border-amber-200 dark:border-amber-900/30"
      >
        <div class="flex items-center justify-between mb-2">
          <div
          ...
          <span class="text-xs font-medium text-amber-600 dark:text-amber-400">
            {{
              kpis.totalRevenue > 0
                ? ((kpis.lightningSales / kpis.totalRevenue) * 100).toFixed(0)
                : 0
            }}%
          </span>
        </div>
        <p class="text-xs text-amber-600 dark:text-amber-400">
          {{ t("dashboard.lightningSales") }}
        </p>
        <p
          class="text-xl font-semibold text-amber-700 dark:text-amber-300 tabular-nums"
        >

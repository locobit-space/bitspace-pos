...
<template>
  ...
  >
    <template #content>
      <div class="flex flex-col h-full bg-white dark:bg-gray-900 w-full max-w-xl">
        <!-- Header -->
        <div class="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-xl bg-primary-500/10 flex items-center justify-center">
              <UIcon name="i-heroicons-magnifying-glass-circle" class="w-5 h-5 text-primary-600 dark:text-primary-400" />
            </div>
            <div>
        ...
        <div class="flex border-b border-gray-200 dark:border-gray-700">
          <button
            v-for="tab in [
              { id: 'search', icon: 'i-heroicons-magnifying-glass', label: t('products.lookup.search') },
              { id: 'barcode', icon: 'i-heroicons-qr-code', label: t('products.lookup.barcode') },
              { id: 'categories', icon: 'i-heroicons-squares-2x2', label: t('products.lookup.browse') },
            ]"
            :key="tab.id"
        ...
        <div class="flex-1 overflow-y-auto p-4">
          ...
          <div v-if="activeTab === 'search'" class="space-y-4">
            ...
            <!-- Loading -->
            <div v-if="productLookup.isSearching.value" class="flex items-center justify-center py-8">
              <UIcon name="i-heroicons-arrow-path" class="w-6 h-6 animate-spin text-primary-500" />
            </div>

            <!-- Error -->
            <div v-else-if="productLookup.searchError.value" class="text-center py-8 text-red-500">
              <UIcon name="i-heroicons-exclamation-circle" class="w-8 h-8 mx-auto mb-2" />
              <p>{{ productLookup.searchError.value }}</p>
            </div>

            <!-- Results -->
            <div v-else-if="productLookup.searchResults.value.length > 0" class="space-y-3">
              <p class="text-sm text-gray-500">
                {{ t('products.lookup.showingOf', { shown: productLookup.searchResults.value.length, total: productLookup.searchTotal.value }) }}
              </p>
              <div
                ...
                :class="selectedProducts.has(product.id)
                  ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20'
                  : 'border-gray-200 dark:border-gray-700 hover:border-primary-300'"
                @click="toggleSelect(product)"
              >
                <!-- Image -->
                <div class="w-14 h-14 rounded-lg bg-gray-100 dark:bg-gray-800 flex-shrink-0 overflow-hidden">
                  <img
                    v-if="product.image"
                    :src="product.image"
                    :alt="product.name"
                  ...
                  />
                  <UIcon v-else name="i-heroicons-photo" class="w-6 h-6 m-4 text-gray-400" />
                </div>
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p v-if="product.brand" class="text-sm text-gray-500 truncate">
                    {{ product.brand }}
                  </p>
                  <div class="flex items-center gap-2 mt-1">
                    <code v-if="product.barcode" class="text-xs bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded">
                      {{ product.barcode }}
                    </code>
              ...
              <div v-if="productLookup.hasMore.value" class="pt-2">
                ...
                >
                  <UIcon v-if="!productLookup.isLoadingMore.value" name="i-heroicons-arrow-down" class="w-4 h-4 mr-2" />
                  {{ productLookup.isLoadingMore.value ? t('products.lookup.loading') : t('products.lookup.loadMore') }}
                </UButton>
              </div>
            </div>
          ...
          <div v-if="activeTab === 'barcode'" class="space-y-4">
            <div class="text-center py-4">
              <div class="w-20 h-20 mx-auto mb-4 rounded-2xl bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
                <UIcon name="i-heroicons-qr-code" class="w-10 h-10 text-gray-400" />
              </div>
            ...
            <!-- Barcode Result -->
            <div v-if="productLookup.searchResults.value.length > 0 && activeTab === 'barcode'" class="mt-4">
              <div
                v-for="product in productLookup.searchResults.value"
                :key="product.id"
              ...
              >
                <div class="flex items-start gap-4">
                  <div class="w-20 h-20 rounded-lg bg-white dark:bg-gray-800 overflow-hidden flex-shrink-0">
                    <img
                      v-if="product.image"
                      :src="product.image"
                      :alt="product.name"
                  ...
                  <div class="flex-1">
                    ...
                    <p v-if="product.brand" class="text-sm text-gray-600 dark:text-gray-400">
                      {{ product.brand }}
                    </p>
          ...
          <div v-if="activeTab === 'categories'" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <span class="font-medium text-gray-900 dark:text-white group-hover:text-primary-600">
                  {{ getCategoryName(cat) }}
                </span>
              </button>
            </div>
        ...
        <!-- Footer -->
        <div v-if="selectedProducts.size > 0" class="p-4 border-t border-gray-200 dark:border-gray-700">
          <UButton
            block
            color="primary"
            size="lg"
          ...
          >
            {{ t('products.lookup.importSelected', { count: selectedProducts.size }) }}
          </UButton>
        </div>
      </div>
    </template>

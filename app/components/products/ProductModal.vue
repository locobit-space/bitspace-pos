        ...
        <header class="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-6 py-4 shrink-0">
          <div class="flex items-center justify-between max-w-6xl mx-auto">
            <div class="flex items-center gap-4">
              <div class="w-14 h-14 bg-gray-100 dark:bg-gray-800 rounded-2xl flex items-center justify-center overflow-hidden border-2 border-gray-200 dark:border-gray-700 shadow-sm">
                <img
                ...
                >
                <span v-else-if="form.image" class="text-3xl">{{ form.image }}</span>
              ...
              <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">
                  {{ form.name || (t('products.newProductHint') || 'Fill in product details below') }}
                </p>
              </div>
            </div>
            <UButton
        ...
        <div class="flex-1 overflow-y-auto">
          <div class="max-w-6xl mx-auto p-6">
            <UForm :schema="productSchema" :state="form" @submit="handleSubmit">
              <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                  ...
                  <ProductsFormCardInventory
                    v-if="form.productType === 'good' || form.productType === 'bundle'"
                    v-model="form"
                  ...
                  <ProductsFormCardExpiry
                    v-if="(form.productType === 'good' || form.productType === 'bundle') && form.trackStock"
                    v-model="form"
                  ...
                  <!-- No Stock Info -->
                  <div 
                    v-if="form.productType === 'service' || form.productType === 'digital' || form.productType === 'subscription'"
                    ...
                    <div class="flex items-start gap-4">
                      ...
                      <div>
                        <h4 class="font-medium text-blue-900 dark:text-blue-100 mb-1">
                          {{ t('products.noStockTitle') || 'No Inventory Tracking' }}
                        </h4>
                        <p class="text-sm text-blue-700 dark:text-blue-300">
                          {{ t('products.noStockTracking') || 'Stock tracking is not available for this product type.' }}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <!-- Footer -->
              <div class="mt-8 flex items-center justify-between p-4 bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm">
                <div class="text-sm text-gray-500">
                  <span v-if="isEditing && product">
                    {{ t('common.lastUpdated') || 'Last updated' }}: {{ new Date(product.updatedAt).toLocaleDateString() }}
                  </span>
                  <span v-else>
                    {{ t('products.createNewHint') || 'Fill in the required fields to create a new product' }}
                  </span>
                </div>

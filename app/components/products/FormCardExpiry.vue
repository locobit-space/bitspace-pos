...
const storageTypeOptions = [
  { value: 'ambient', label: 'Ambient', icon: 'i-heroicons-sun' },
  { value: 'refrigerated', label: 'Refrigerated', icon: 'i-heroicons-snowflake' },
  { value: 'frozen', label: 'Frozen', icon: 'i-heroicons-cube' },
  { value: 'controlled', label: 'Controlled', icon: 'i-heroicons-shield-check' },
];
</script>
      ...
      <template v-if="form.hasExpiry">
        ...
        <div class="flex items-center justify-between p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-gray-500">
                {{ t('products.requiresExpiryDateHint') || 'Must enter expiry when receiving stock' }}
              </p>
            </div>
          </div>
        ...
        <div class="grid grid-cols-2 gap-4 pt-2">
          <UFormField :label="t('products.defaultShelfLifeDays') || 'Shelf Life (days)'" name="defaultShelfLifeDays">
            <UInput
              v-model.number="form.defaultShelfLifeDays"
              type="number"
              placeholder="30"
            >
              <template #trailing>
                <span class="text-xs text-gray-500">{{ t('common.days') || 'days' }}</span>
              </template>
            </UInput>
          </UFormField>

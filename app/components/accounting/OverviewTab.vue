...
const financialHealth = computed(() => {
  const { revenue, expenses, profit, accountsReceivable, accountsPayable } = props.financialOverview;
  if (revenue === 0 && expenses === 0) return { score: 50, label: 'neutral', color: 'gray' };
...
<template>
  <div class="space-y-6">
    ...
    <div 
      ...
      :class="{
        'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800': financialHealth.color === 'green',
        'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800': financialHealth.color === 'blue',
        'bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-800': financialHealth.color === 'yellow',
        'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800': financialHealth.color === 'red',
        'bg-gray-50 dark:bg-gray-800/20 border-gray-200 dark:border-gray-700': financialHealth.color === 'gray',
      }"
    >
      <div class="flex items-center gap-4">
        <div 
          ...
          :class="{
            ...
            'bg-yellow-100 dark:bg-yellow-800': financialHealth.color === 'yellow',
            'bg-red-100 dark:bg-red-800': financialHealth.color === 'red',
            'bg-gray-100 dark:bg-gray-700': financialHealth.color === 'gray',
          }"
        >
          <UIcon 
            :name="financialHealth.score >= 60 ? 'i-heroicons-check-circle' : 'i-heroicons-exclamation-triangle'" 
            class="w-7 h-7"
            :class="{
              'text-green-600 dark:text-green-400': financialHealth.color === 'green',
              'text-blue-600 dark:text-blue-400': financialHealth.color === 'blue',
              'text-yellow-600 dark:text-yellow-400': financialHealth.color === 'yellow',
              'text-red-600 dark:text-red-400': financialHealth.color === 'red',
              'text-gray-600 dark:text-gray-400': financialHealth.color === 'gray',
            }"
          />
        </div>
        <div>
          <p class="text-sm text-muted">Financial Health</p>
          <p class="text-xl font-bold capitalize" :class="`text-${financialHealth.color}-600 dark:text-${financialHealth.color}-400`">
            {{ financialHealth.label === 'needsAttention' ? 'Needs Attention' : financialHealth.label }}
          </p>
        </div>
      </div>
      <div class="text-right">
        <p class="text-3xl font-bold" :class="`text-${financialHealth.color}-600 dark:text-${financialHealth.color}-400`">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      <!-- Revenue Card -->
      <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-green-500 to-emerald-600 p-5 text-white shadow-lg">
        <div class="absolute top-0 right-0 w-20 h-20 bg-white/10 rounded-full -mr-10 -mt-10"></div>
        <UIcon name="i-heroicons-arrow-trending-up" class="w-8 h-8 mb-3 opacity-80" />
        <p class="text-sm opacity-80">{{ t('accounting.revenue') }}</p>
        <p class="text-2xl font-bold mt-1">{{ formatCurrency(financialOverview.revenue) }}</p>
      </div>

      <!-- Expenses Card -->
      <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-red-500 to-rose-600 p-5 text-white shadow-lg">
        <div class="absolute top-0 right-0 w-20 h-20 bg-white/10 rounded-full -mr-10 -mt-10"></div>
        <UIcon name="i-heroicons-arrow-trending-down" class="w-8 h-8 mb-3 opacity-80" />
        <p class="text-sm opacity-80">{{ t('accounting.expenseLabel') }}</p>
        <p class="text-2xl font-bold mt-1">{{ formatCurrency(financialOverview.expenses) }}</p>
      </div>

      <!-- Profit Card -->
      <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 p-5 text-white shadow-lg">
        <div class="absolute top-0 right-0 w-20 h-20 bg-white/10 rounded-full -mr-10 -mt-10"></div>
        <UIcon name="i-heroicons-chart-bar" class="w-8 h-8 mb-3 opacity-80" />
        <p class="text-sm opacity-80">{{ t('accounting.profit') }}</p>
        <p class="text-2xl font-bold mt-1">{{ formatCurrency(financialOverview.profit) }}</p>
        <p class="text-xs mt-1 opacity-70">{{ financialOverview.profitMargin }}% {{ t('accounting.margin') }}</p>
      </div>
      ...
      <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 p-5 text-white shadow-lg">
        ...
        <p class="text-2xl font-bold mt-1">{{ financialOverview.lightningBalance.toLocaleString() }} sats</p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <UCard class="hover:shadow-md transition-shadow">
        <div class="flex items-center gap-4">
          ...
          <div>
            <p class="text-sm text-muted">{{ t('accounting.accountsReceivable') }}</p>
            <p class="text-xl font-bold text-green-600 dark:text-green-400">{{ formatCurrency(financialOverview.accountsReceivable) }}</p>
          </div>
        </div>
      </UCard>
      ...
      <UCard class="hover:shadow-md transition-shadow">
        <div class="flex items-center gap-4">
          <div class="p-3 bg-red-100 dark:bg-red-900/30 rounded-xl">
            <UIcon name="i-heroicons-arrow-up-tray" class="w-6 h-6 text-red-600 dark:text-red-400" />
          </div>
          <div>
            <p class="text-sm text-muted">{{ t('accounting.accountsPayable') }}</p>
            <p class="text-xl font-bold text-red-600 dark:text-red-400">{{ formatCurrency(financialOverview.accountsPayable) }}</p>
          </div>
        </div>
      </UCard>
      ...
      <UCard class="hover:shadow-md transition-shadow">
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <p class="text-xl font-bold text-blue-600 dark:text-blue-400">{{ formatCurrency(financialOverview.cashBalance) }}</p>
          </div>
        </div>
      </UCard>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <NuxtLink to="/accounting/accounts">
        <UCard class="hover:shadow-lg transition-all cursor-pointer hover:scale-[1.02] hover:border-blue-300 dark:hover:border-blue-700">
          <div class="flex items-center gap-3">
            <div class="p-2 bg-blue-100 dark:bg-blue-900/30 rounded-lg">
              <UIcon name="i-heroicons-rectangle-stack" class="w-5 h-5 text-blue-600 dark:text-blue-400" />
            </div>
      ...
      <NuxtLink to="/accounting/ledger">
        <UCard class="hover:shadow-lg transition-all cursor-pointer hover:scale-[1.02] hover:border-purple-300 dark:hover:border-purple-700">
          <div class="flex items-center gap-3">
      ...
      <NuxtLink to="/accounting/expenses">
        <UCard class="hover:shadow-lg transition-all cursor-pointer hover:scale-[1.02] hover:border-red-300 dark:hover:border-red-700">
          <div class="flex items-center gap-3">
      ...
      <NuxtLink to="/invoicing">
        <UCard class="hover:shadow-lg transition-all cursor-pointer hover:scale-[1.02] hover:border-green-300 dark:hover:border-green-700">
          <div class="flex items-center gap-3">
    ...
    <UCard>
      ...
      <div v-if="chartOfAccounts.length === 0" class="text-center py-12">
        ...
        <p class="text-sm text-muted mb-4">Set up your chart of accounts to start tracking finances</p>
        <NuxtLink to="/accounting/accounts">
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <span v-if="account.nameLao" class="text-sm text-muted ml-2">{{ account.nameLao }}</span>
            </div>
            <UBadge :class="getTypeColor(account.type)" variant="subtle" size="xs">
              {{ t(`accounting.types.${account.type}`) }}
            </UBadge>
          </div>
          <span class="font-mono font-medium" :class="account.balance >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">

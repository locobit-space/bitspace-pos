...
<template>
  <div class="space-y-6">
    ...
    <div class="p-4 rounded-2xl border-2 flex items-center justify-between" :class="{
      ...
      <div class="flex items-center gap-4">
        <div class="w-14 h-14 rounded-full flex items-center justify-center" :class="{
          ...
          <UIcon :name="financialHealth.score >= 60
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
      ...
      <UCard
        class="hover:shadow-lg transition-all cursor-pointer hover:scale-[1.02] hover:border-emerald-300 dark:hover:border-emerald-700"
        @click="emit('add-income')">
        <div class="flex items-center gap-3">
          <div class="p-2 bg-emerald-100 dark:bg-emerald-900/30 rounded-lg">
            <UIcon name="i-heroicons-arrow-trending-up" class="w-5 h-5 text-emerald-600 dark:text-emerald-400" />
          </div>
          <span class="font-medium">{{ t("accounting.income.addIncome") }}</span>
        </div>
      </UCard>
    </div>
    ...
    <UCard>
      ...
      <div v-else class="divide-y divide-gray-100 dark:divide-gray-800 max-h-96 overflow-y-auto">
        <div v-for="account in chartOfAccounts.slice(0, 10)" :key="account.code"
          ...
          <div class="flex items-center gap-3">
            <span class="font-mono text-sm px-2.5 py-1 rounded-lg" :class="getTypeBgColor(account.type)">
              {{ account.code }}
            </span>
            <div>
          ...
          <span class="font-mono font-medium" :class="account.balance >= 0
            ? 'text-green-600 dark:text-green-400'

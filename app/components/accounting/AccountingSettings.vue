...
<template>
  <div class="max-w-2xl mx-auto space-y-6">
    <UCard>
      ...
      <div class="space-y-6">
        ...
        <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
          <div>
            <p class="font-medium text-gray-900 dark:text-white">Auto Journal Entries</p>
            <p class="text-sm text-gray-500">
              Automatically create journal entries from POS sales, stock adjustments, and memberships
            </p>
          </div>
          ...
          <div class="flex items-start gap-3">
            ...
            <div class="text-sm">
              <p class="font-medium text-blue-700 dark:text-blue-300">
                {{ localStandard === 'lao' ? 'Lao Country Standard (LAO-GAAP)' : 'Global Standard (IFRS)' }}
              </p>
              <ul class="mt-1 text-blue-600 dark:text-blue-400 space-y-1">
                <li v-if="localStandard === 'lao'">• 4-digit account codes (1000-6000)</li>
                <li v-if="localStandard === 'lao'">• LAK as primary currency</li>
                <li v-if="localStandard === 'lao'">• Lao language account names (ບັນຊີ)</li>
                <li v-if="localStandard === 'global'">• 5-digit account codes (10000-69000)</li>
                <li v-if="localStandard === 'global'">• Multi-currency support</li>
                <li v-if="localStandard === 'global'">• IFRS compliant structure</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
          <div>
            <p class="font-medium text-gray-900 dark:text-white">Auto-Post Journals</p>
            <p class="text-sm text-gray-500">
    ...
    <UCard>
      <template #header>
        <h3 class="font-semibold text-gray-900 dark:text-white">Current Chart of Accounts</h3>
      </template>
      <div class="text-sm text-gray-600 dark:text-gray-400">
        <p>You have <strong>{{ accountsCount }}</strong> accounts loaded.</p>
        <p class="mt-2">Standard: <strong>{{ localStandard === 'lao' ? 'Lao Country (LAO-GAAP)' : 'Global (IFRS)' }}</strong></p>
        <p>VAT Rate: <strong>{{ localVatRate }}%</strong></p>
        <p>Auto-Post: <strong>{{ localAutoPost ? 'Enabled' : 'Disabled' }}</strong></p>
      </div>
    </UCard>
  </div>
</template>

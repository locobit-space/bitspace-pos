  ...
  <div class="space-y-6">
    <UCard>
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            <div class="p-2 bg-indigo-100 dark:bg-indigo-900/30 rounded-lg">
              <UIcon name="i-heroicons-book-open" class="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
            </div>
      ...
      <div v-if="entries.length === 0" class="text-center py-12">
        ...
        <p class="text-sm text-muted mb-4">Start tracking your financial transactions</p>
          ...
          >
            ...
            <div class="flex items-center gap-4">
              <div class="text-right text-sm">
                <span class="text-green-600 dark:text-green-400 font-mono">{{ formatCurrency(getTotalDebit(entry)) }}</span>
                <span class="mx-1 text-muted">/</span>
                <span class="text-red-600 dark:text-red-400 font-mono">{{ formatCurrency(getTotalCredit(entry)) }}</span>
              </div>
              <!-- Balance Indicator -->
              <UBadge 
          ...
          >
            ...
            >
              <table class="w-full text-sm">
                <thead>
                  <tr class="text-left text-muted border-b border-gray-200 dark:border-gray-700">
                    <th class="pb-2 font-medium">{{ t('accounting.account') }}</th>
                    <th class="pb-2 text-right font-medium">{{ t('accounting.debit') }}</th>
                    <th class="pb-2 text-right font-medium">{{ t('accounting.credit') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr 
                  ...
                  >
                    <td class="py-2">
                      <span class="text-gray-700 dark:text-gray-300">{{ line.account }}</span>
                    </td>
                    <td class="py-2 text-right font-mono">
                      <span v-if="line.debit > 0" class="text-green-600 dark:text-green-400 font-medium">
                        {{ formatCurrency(line.debit) }}
                      </span>
                    </td>
                    <td class="py-2 text-right font-mono">
                      <span v-if="line.credit > 0" class="text-red-600 dark:text-red-400 font-medium">
                ...
                <tfoot>
                  <tr class="border-t-2 border-gray-300 dark:border-gray-600 font-bold">
                    ...
                    <td class="pt-2 text-right font-mono text-green-600 dark:text-green-400">
                      {{ formatCurrency(getTotalDebit(entry)) }}
                    </td>
                    <td class="pt-2 text-right font-mono text-red-600 dark:text-red-400">

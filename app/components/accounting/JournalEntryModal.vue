      ...
      <div class="space-y-4">
        <div class="grid grid-cols-2 gap-4">
          <UFormField :label="t('accounting.date')">
            <UInput :model-value="form.date" type="date" @update:model-value="updateFormField('date', $event)" />
          </UFormField>
          <UFormField :label="t('accounting.description')">
            <UInput :model-value="form.description" @update:model-value="updateFormField('description', $event)" />
          </UFormField>
        </div>
        ...
        <div class="space-y-2">
          ...
          <div v-for="(line, idx) in form.lines" :key="idx" class="grid grid-cols-3 gap-2">
            <UInput :model-value="line.account" :placeholder="t('accounting.account')"
              @update:model-value="updateLine(idx, 'account', $event)" />
            <UInput :model-value="line.debit" type="number" :placeholder="t('accounting.debit')"
              @update:model-value="updateLine(idx, 'debit', Number($event))" />
            <UInput :model-value="line.credit" type="number" :placeholder="t('accounting.credit')"
          ...
          <!-- Balance indicator -->
          <div class="flex justify-between items-center pt-2 border-slate-200 dark:border-slate-700 border-t">

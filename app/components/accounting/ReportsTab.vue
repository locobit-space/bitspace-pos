...
const dateRange = reactive({
  start: new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().split('T')[0] as string,
  ...
  <div class="space-y-6">
      ...
      <div class="flex flex-wrap items-center justify-between gap-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-sm text-muted">{{ t('accounting.reports.exportAsCSV') }}</p>
          </div>
        </div>
        ...
        >
          <div class="flex items-start justify-between">
            <div class="flex items-center gap-4">
              <div class="p-3 rounded-lg transition-transform group-hover:scale-110" :class="report.bgColor">
                <UIcon :name="report.icon" class="w-6 h-6" :class="report.iconColor" />
              </div>
              <div>
                <h4 class="font-medium group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <NuxtLink to="/accounting/ledger">
        <UCard class="hover:shadow-lg transition-all cursor-pointer text-center py-4 hover:scale-[1.02]">
      ...
      <NuxtLink to="/accounting/expenses">
        <UCard class="hover:shadow-lg transition-all cursor-pointer text-center py-4 hover:scale-[1.02]">
          <UIcon name="i-heroicons-credit-card" class="w-8 h-8 text-primary mx-auto mb-2" />
          <p class="font-medium">{{ t('accounting.expenses.title') }}</p>
        </UCard>
      </NuxtLink>
      <NuxtLink to="/invoicing">
        <UCard class="hover:shadow-lg transition-all cursor-pointer text-center py-4 hover:scale-[1.02]">
          <UIcon name="i-heroicons-document-text" class="w-8 h-8 text-primary mx-auto mb-2" />
